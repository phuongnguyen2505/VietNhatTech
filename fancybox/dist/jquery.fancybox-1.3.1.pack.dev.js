...
(function (b) {
  var m,
    ...
    if (e.type) {
      f = e.type;
      if (!d) d = e.content;
    } else if (e.content) f = "html";else if (d) {
      if (d.match(G)) f = "image";else if (d.match(S)) f = "swf";else if (b(a).hasClass("iframe")) f = "iframe";else if (d.match(/#/)) {
    ...
    if (!h) {
      ...
      if (b.isArray(a)) {
        for (var f = 0, o = a.length; f < o; f++) {
          if (_typeof(a[f]) == "object") b(a[f]).data("fancybox", b.extend({}, d, a[f]));else a[f] = b({}).data("fancybox", b.extend({
            content: a[f]
          }, d));
        }
        ...
        if (_typeof(a) == "object") b(a).data("fancybox", b.extend({}, d, a));else a = b({}).data("fancybox", b.extend({
          content: a
        }, d));
        q.push(a);
      }
