...
var _require$doc$builders = require("prettier").doc.builders,
    ...
    cursor = _require$doc$builders.cursor,
...
function handleRetifComments(enclosingNode, precedingNode, followingNode, comment, text, options) {
  var isSameLineAsPrecedingNode = precedingNode && !hasNewlineInRange(text, options.locEnd(precedingNode), options.locStart(comment));
...
function handleFunction(text, enclosingNode, followingNode, comment, options) {
  if (enclosingNode && (enclosingNode.kind === "function" || enclosingNode.kind === "method")) {
    ...
    for (var i = 0; i < enclosingNode.arguments.length; i++) {
      argumentsLocEnd = options.locEnd(enclosingNode.arguments[i]) > argumentsLocEnd ? options.locEnd(enclosingNode.arguments[i]) : argumentsLocEnd;
    }
...
function getCommentChildNodes(node) {
  ...
  var getChildNodes = function getChildNodes(node) {
    return Object.keys(node).filter(function (n) {
      return n !== "kind" && n !== "loc" && n !== "errors" && n !== "extra" && n !== "comments" && n !== "leadingComments" && n !== "enclosingNode" && n !== "precedingNode" && n !== "followingNode";
    }).map(function (n) {
      return node[n];
    });
  };
...
function printLeadingComment(commentPath, print, options) {
  ...
  if (isBlock) {
    return [contents, hasNewline(options.originalText, options.locEnd(comment)) ? hardline : " "];
  }
