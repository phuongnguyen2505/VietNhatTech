...
(function ($) {
  $.fn.ashtml5lightbox = function (options) {
    ...
    inst.createMarkup = function () {
      ...
      if (inst.options.titlestyle == "inside") {
        inst.$elemData.css(_defineProperty({
          position: "absolute",
    ...
    inst.showNavigation = function () {
      ...
      for (i = 0; i < inst.elemArray.length; i++) {
        if (group == inst.elemArray[i][ELEM_GROUP]) if (inst.elemArray[i][ELEM_THUMBNAIL] && inst.elemArray[i][ELEM_THUMBNAIL].length > 0) {
          showNav = true;
          break;
        }
      }
...
(function ($) {
  $.fn.asHTML5VideoControls = function (skinFolder, parentInst, hidecontrols, hideplaybutton) {
    ...
    var userActivate = function userActivate() {
      userActive = true;
    };
    ...
    if (!isIPhone && ($videoObj.get(0).requestFullscreen || $videoObj.get(0).webkitRequestFullScreen || $videoObj.get(0).mozRequestFullScreen || $videoObj.get(0).webkitEnterFullScreen || $videoObj.get(0).msRequestFullscreen)) {
      var switchScreen = function switchScreen(fullscreen) {
        if (fullscreen) {
      ...
      var switchScreenCSS = function switchScreenCSS(fullscreen) {
        $videoControls.css({
    ...
    var calcTimeFormat = function calcTimeFormat(seconds) {
    ...
    var onVideoUpdate = function onVideoUpdate() {
...
(function ($) {
  $.fn.amazingslider = function (options) {
    ...
    var AmazingSlider = function AmazingSlider(container, options, textoptions, id) {
    ...
    AmazingSlider.prototype = {
      ...
      getYoutubeParams: function getYoutubeParams(href) {
        var result = {};
        if (href.indexOf("?") < 0) return result;
        var params = href.substring(href.indexOf("?") + 1).split("&");
      ...
      showTexteffect: function showTexteffect(event) {
        var sliderid = $(this).data("sliderid");
        $(".amazingslider-text-box-" + sliderid, this).fadeIn();
      },
      hideTexteffect: function hideTexteffect(event) {
        var sliderid = $(this).data("sliderid");
      ...
      createText: function createText() {
        ...
        this.container.bind("amazingslider.switchtext", function (event, prev, cur) {
          ...
          } else if (instance.textoptions.textstyle == "dynamic") if (!instance.elemArray[cur][ELEM_TITLE] && !instance.elemArray[cur][ELEM_DESCRIPTION] && !instance.elemArray[cur][ELEM_BUTTON]) $textWrapper.fadeOut();else $textWrapper.fadeOut(function () {
            $title.html(instance.elemArray[cur][ELEM_TITLE]);
            if (instance.elemArray[cur][ELEM_DESCRIPTION].length > 0) $description.html(instance.elemArray[cur][ELEM_DESCRIPTION]).css({
              "display": "block"
            });else $description.css({
      ...
      resizeFullscreenImage: function resizeFullscreenImage($imgElem) {
        ...
        } else if (this.options.scalemode == "fit") this.curImageMarginTop = (sliderRatio - imgRatio) * 50;else {
      ...
      slideRun: function slideRun(index, playVideo, transEffect) {
      ...
      showImage: function showImage(slideDirection, transEffect) {
        var instance = this;
        var imgLoader = new Image();
        $(imgLoader).load(function () {
    ...
    var asbts = function asbts(string) {
      var ret = "";
