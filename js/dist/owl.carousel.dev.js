...
(function ($, window, document, undefined) {
  ...
  Owl.prototype.init = function () {
    ...
    this.trigger('initialize'); // Add base class
    ...
    this.width.prevWindow = this.viewport(); // create stage object
    ...
    this.trigger('initialized'); // attach custom control events
  ...
  Owl.prototype.setResponsiveOptions = function () {
    if (!this.options.responsive) {
      ...
      delete this.settings.responsive; // Responsive Class
  ...
  Owl.prototype.optionsLogic = function () {
    // Toggle Center class
    this.dom.$el.toggleClass('owl-center', this.settings.center); // if items number is less than in body
  ...
  Owl.prototype.cloneItemContainer = function (item) {
    var $clone = item.clone(true, true).addClass('cloned'); // somehow data references the same object
  ...
  Owl.prototype.loopClone = function () {
    ...
    var append,
        ...
        last = this.num.oItems - 1; // if neighbour margin then add one more duplicat
  ...
  Owl.prototype.calculate = function () {
    ...
    this.width.el = this.dom.$el.width() - this.settings.stagePadding * 2; // to check

    this.width.view = this.dom.$el.width(); // calculate width minus addition margins

    elMinusMargin = this.width.el - this.settings.margin * (this.settings.items === 1 ? 0 : this.settings.items - 1); // calculate element width and item width
    ...
    this._coordinates = [];
    this.num.merged = []; // item distances
    ...
    for (i = 0; i < this.num.items; i++) {
      ...
      iWidth = this.width.item * this.num.merged[i]; // autoWidth item size
      ...
      this._coordinates.push(dist); // update item data
  ...
  Owl.prototype.refresh = function () {
    ...
    this.trigger('refresh'); // Update Options for given width

    this.setResponsiveOptions(); // update info about local content
  ...
  Owl.prototype.updateActiveItems = function () {
    ...
    var i, j, item, ipos, iwidth, outsideView; // clear states
  ...
  Owl.prototype.onDragStart = function (event) {
    ...
    pageY = isTouchEvent ? event.targetTouches[0].pageY : ev.pageY || ev.clientY; // get stage position left
  ...
  Owl.prototype.onDragMove = function (event) {
    ...
    ev = event.originalEvent || event || window.event; // if is 'touchstart'

    isTouchEvent = ev.type == 'touchmove';
    pageX = isTouchEvent ? ev.targetTouches[0].pageX : ev.pageX || ev.clientX;
    pageY = isTouchEvent ? ev.targetTouches[0].pageY : ev.pageY || ev.clientY; // Drag Direction
  ...
  Owl.prototype.onDragEnd = function () {
    ...
    this.current(closest); // if pullDrag is off then fire transitionEnd event manually when stick
    // to border
  ...
  Owl.prototype.getTransformProperty = function () {
    ...
    transform = window.getComputedStyle(this.dom.stage, null).getPropertyValue(this.vendorName + 'transform'); // var transform = this.dom.$stage.css(this.vendorName + 'transform')
  ...
  Owl.prototype.addItem = function (content, position) {
    ...
    position = position || 0; // wrap content

    this.initializeItemContainer($item, content); // if carousel is empty then append item
...
(function ($, window, document, undefined) {
  ...
  Video.prototype.getVideoInfo = function (videoEl, item) {
    ...
    dimensions = width && height ? 'style="width:' + width + 'px;height:' + height + 'px;"' : ''; // wrap video content into owl-video-wrapper div
...
(function ($, window, document, undefined) {
  ...
  var Navigation = function Navigation(carousel) {
    ...
    this.core.options = $.extend({}, Navigation.Defaults, this.core.options); // register event handlers
  ...
  Navigation.prototype.initialize = function () {
    var $container,
        override,
        options = this.core.settings; // create the indicator template
...
(function ($, window, document, undefined) {
  ...
  var Hash = function Hash(carousel) {
    ...
    this.core.options = $.extend({}, Hash.Defaults, this.core.options); // register the event handlers

    this.$element.on(this.handlers); // register event listener for hash navigation
