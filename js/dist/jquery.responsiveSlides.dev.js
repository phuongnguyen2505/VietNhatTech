  ...
  $.fn.responsiveSlides = function (options) {
    ...
    _this.RS_ShowPhoto = function (next) {
      ...
      next.fadeIn(settings.effectTransition, function () {
        ...
        if (_int == null && settings.autoStart) {
          _this.RS_Start();
        }
      });
    ...
    $('img', $('.tgtimg', _this)).onImagesLoad(function () {
      ...
      if (p.active) {
        ...
        var elm = '';
        $('img', $('.tgtimg', _this)).each(function (i, e) {
          elm += '<div' + (p.dotStyle == '' ? ' style="float:left; height:15px; width:15px; background:#fff; margin-left:2px; cursor:pointer"' : ' class="' + p.dotStyle + '"') + '></div>';
        });
        ...
        $('div', $('.pagination', _this)).mouseover(function () {
          if (p.dotStyleHover != '') {
            $(this).addClass(p.dotStyleHover);
          } else {
            $(this).css('background', '#CCC');
          }
        }).mouseout(function () {
          if (p.dotStyleHover != '') {
            $(this).removeClass(p.dotStyleHover);
          } else {
