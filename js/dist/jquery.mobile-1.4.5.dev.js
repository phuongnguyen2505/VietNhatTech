...
(function (root, doc, factory) {
...
})(void 0, document, function (jQuery, window, document, undefined) {
  (function ($) {
    $.mobile = {};
  })(jQuery);
  /*!
  ...
  (function ($, window, undefined) {
    ...
    var compensateToolbars = function compensateToolbars(page, desiredHeight) {
      ...
      noPadders = function noPadders() {
        var theElement = $(this),
            widgetOptions = $.mobile.toolbar && theElement.data("mobile-toolbar") ? theElement.toolbar("option") : {
        ...
        return !(widgetOptions.position === "fixed" && widgetOptions.updatePagePadding === true);
          ...
          internalFooters = page.children(":jqmData(role='footer')"); // If we have no internal headers, but we do have external headers, then their height
    ...
    $.extend($.mobile, {
      ...
      silentScroll: function silentScroll(ypos) {
        if ($.type(ypos) !== "number") {
      ...
      getClosestBaseUrl: function getClosestBaseUrl(ele) {
      ...
      removeActiveLinkClass: function removeActiveLinkClass(forceRemoval) {
      ...
      // as possible.
      getInheritedTheme: function getInheritedTheme(el, defaultTheme) {
        var e = el[0],
            ltr = "",
      ...
      enhanceable: function enhanceable(elements) {
        return this.haveParents(elements, "enhance");
      },
      hijackable: function hijackable(elements) {
        return this.haveParents(elements, "ajax");
      },
      haveParents: function haveParents(elements, attr) {
      ...
      resetActivePageHeight: function resetActivePageHeight(height) {
        var page = $("." + $.mobile.activePageClass),
            pageHeight = page.height(),
            pageOuterHeight = page.outerHeight(true);
        height = compensateToolbars(page, typeof height === "number" ? height : $.mobile.getScreenHeight()); // Remove any previous min-height setting

        page.css("min-height", ""); // Set the minimum height only if the height as determined by CSS is insufficient
      ...
      loading: function loading() {
        ...
        returnValue = loader.loader.apply(loader, arguments); // Make sure the loader is retained for future calls to this function.
    ...
    $.fn.extend({
      ...
      enhanceWithin: function enhanceWithin() {
        var index,
            ...
            that = this; // Add no js class to elements
        ...
        $.each($.mobile.widgets, function (name, constructor) {
          ...
          if (constructor.initSelector) {
            ...
            var elements = $.mobile.enhanceable(that.find(constructor.initSelector)); // If any matching elements remain filter ones with keepNativeSelector
      ...
      addDependents: function addDependents(newDependents) {
  ...
  (function ($, window, undefined) {
    ...
    $.extend($.mobile, {
      ...
      getAttribute: function getAttribute(element, key) {
        var data;
      ...
      nsNormalize: function nsNormalize(prop) {
      ...
      closestPageData: function closestPageData($target) {
  ...
  (function ($, undefined) {
    var rcapitals = /[A-Z]/g,
        replaceFunction = function replaceFunction(c) {
  ...
  (function ($) {
    ...
    $.widget("mobile.loader", {
      ...
      fakeFixLoader: function fakeFixLoader() {
      ...
      show: function show(theme, msgText, textonly) {
        var textVisible, message, loadSettings;
        this.resetHtml(); // use the prototype options so that people can set them globally at
        ...
        if ($.type(theme) === "object") {
          ...
          loadSettings = this.options; // here we prefer the theme value passed as a string argument, then
        ...
        message = msgText || (loadSettings.text === false ? "" : loadSettings.text); // prepare the dom

        $html.addClass("ui-loading");
        textVisible = loadSettings.textVisible; // add the proper css given the options (theme, text, etc)
        // Force text visibility if the second argument was supplied, or
        // if the text was explicitly set in the object args

        this.element.attr("class", loaderClass + " ui-corner-all ui-body-" + theme + " ui-loader-" + (textVisible || msgText || theme.text ? "verbose" : "default") + (loadSettings.textonly || textonly ? " ui-loader-textonly" : "")); // TODO verify that jquery.fn.html is ok to use in both cases here
        ...
        this.element.appendTo($.mobile.pagecontainer ? $(":mobile-pagecontainer") : $("body")); // check that the loader is visible
  ...
  (function ($, undefined) {
    ...
    }(document); // $.mobile.media uses matchMedia to return a boolean.
  ...
  (function ($, undefined) {
    ...
    nokiaLTE7_3; // inline SVG support test
    ...
    function fixedPosition() {
      ...
      if ( // iOS 4.3 and older : Platform is iPhone/Pad/Touch and Webkit version is less than 534 (ios5)
      (platform.indexOf("iPhone") > -1 || platform.indexOf("iPad") > -1 || platform.indexOf("iPod") > -1) && wkversion && wkversion < 534 || // Opera Mini
      w.operamini && {}.toString.call(w.operamini) === "[object OperaMini]" || operammobilematch && omversion < 7458 || //Android lte 2.1: Platform is Android and Webkit version is less than 533 (Android 2.2)
    ...
    $.extend($.support, {
      ...
      !(window.navigator.userAgent.indexOf("Firefox") >= 0 && window.top !== window) && window.navigator.userAgent.search(/CriOS/) === -1,
    ...
    nokiaLTE7_3 = function () {
      var ua = window.navigator.userAgent; //The following is an attempt to match Nokia browsers that are running Symbian/s60, with webkit, version 7.3 or older
    ...
    nokiaLTE7_3; // Lastly, this workaround is the only way we've found so far to get pre 7.3 Symbian webkit devices
  ...
  (function ($, undefined) {
    var $win = $.mobile.window,
        self,
        dummyFnToInitNavigate = function dummyFnToInitNavigate() {};
    ...
    $.event.special.navigate = self = {
      ...
      isPushStateEnabled: function isPushStateEnabled() {
      ...
      popstate: function popstate(event) {
        ...
        newEvent.originalEvent = event; // NOTE we let the current stack unwind because any assignment to
      ...
      /*, data */
        ...
        newEvent.originalEvent = event; // Trigger the hashchange with state provided by the user
  ...
  (function ($, undefined) {
    ...
    $.mobile.path = path = {
      ...
      getLocation: function getLocation(url) {
        ...
        // authority
        hash = parsedUrl.hash; // mimic the browser with an empty string when the hash is empty

        hash = hash === "#" ? "" : hash;
        return uri.protocol + parsedUrl.doubleSlash + uri.host + ( // The pathname must start with a slash if there's a protocol, because you
        // can't have a protocol followed by a relative path. Also, it's impossible to
        // calculate absolute URLs from relative ones if the absolute one doesn't have
        // a leading "/".
        uri.protocol !== "" && uri.pathname.substring(0, 1) !== "/" ? "/" : "") + uri.pathname + uri.search + hash;
      },
      //return the original document url
      getDocumentUrl: function getDocumentUrl(asParsedObject) {
      ...
      parseUrl: function parseUrl(url) {
        ...
        var matches = path.urlParseRE.exec(url || "") || []; // Create an object that allows the caller to access the sub-matches
      ...
      addSearchParams: function addSearchParams(url, params) {
        var u = path.parseUrl(url),
            p = _typeof(params) === "object" ? $.param(params) : params,
            s = u.search || "?";
      ...
      stripQueryParams: function stripQueryParams(url) {
        return url.replace(/\?.*$/, "");
      },
      ...
      isEmbeddedPage: function isEmbeddedPage(url) {
        var u = path.parseUrl(url); //if the path is absolute, then we need to compare the url against
      ...
      squash: function squash(url, resolutionUrl) {
        var href,
            cleanedUrl,
            search,
            stateIndex,
            ...
            uiState = ""; // produce a url against which we can resolve the provided path
      ...
      isPreservableHash: function isPreservableHash(hash) {
        return hash.replace("#", "").indexOf(this.uiStateKey) === 0;
      },
      // Escape weird characters in the hash if it is to be used as a selector
      hashToSelector: function hashToSelector(hash) {
      ...
      isFirstPageUrl: function isFirstPageUrl(url) {
        ...
        fpId = fp && fp[0] ? fp[0].id : undefined; // The url refers to the first page if the path matches the document and
      ...
      isPermittedCrossDomainRequest: function isPermittedCrossDomainRequest(docUrl, reqUrl) {
    ...
    path.documentBaseDiffers = path.documentUrl.hrefNoHash !== path.documentBase.hrefNoHash; //return the original document base url
  ...
  (function ($, undefined) {
    ...
    $.extend($.mobile.History.prototype, {
      ...
      add: function add(url, data) {
        data = data || {}; //if there's forward history, wipe it
      ...
      find: function find(url, stack, earlyReturn) {
      ...
      closest: function closest(url) {
        var closest,
            a = this.activeIndex; // First, take the slice of the history stack before the current index and search
        // for a url match. If one is found, we'll avoid avoid looking through forward history
        // NOTE the preference for backward history movement is driven by the fact that
        //      most mobile browsers only have a dedicated back button, and users rarely use
        //      the forward button in desktop browser anyhow

        closest = this.find(url, this.stack.slice(0, a)); // If nothing was found in backward history check forward. The `true`
      ...
      direct: function direct(opts) {
        var newActiveIndex = this.closest(opts.url),
            a = this.activeIndex; // save new page index, null check to prevent falsey 0 result
  ...
  (function ($, undefined) {
    ...
    $.extend($.mobile.Navigator.prototype, {
      squash: function squash(url, data) {
        var state,
            href,
            hash = path.isPath(url) ? path.stripHash(url) : url;
        href = path.squash(url); // make sure to provide this information when it isn't explicitly set in the
      ...
      hash: function hash(url, href) {
        var parsed, loc, hash, resolved; // Grab the hash for recording. If the passed url is a path
        ...
        if (loc.pathname + loc.search === parsed.pathname + parsed.search) {
          ...
          resolved = path.parseUrl(href); // If the passed url is a path, make it domain relative and remove any trailing hash
      ...
      go: function go(url, data, noEvents) {
        var state,
            href,
            hash,
            popstateEvent,
            isPopStateEvent = $.event.special.navigate.isPushStateEnabled(); // Get the url as it would look squashed on to the current resolution url

        href = path.squash(url); // sort out what the hash sould be from the url

        hash = this.hash(url, href); // Here we prevent the next hash change or popstate event from doing any
        ...
        window.location.hash = hash; // If popstate is enabled and the browser triggers `popstate` events when the hash
        ...
        if (isPopStateEvent) {
          ...
          this.squash(url, state); // Trigger a new faux popstate event to replace the one that we
      ...
      popstate: function popstate(event) {
        var hash, state; // Partly to support our test suite which manually alters the support
        ...
        if (!event.originalEvent.state && hash) {
          ...
          event.historyState = state; // do not alter history, we've added a new history entry
        ...
        this.history.direct({
          ...
          present: function present(historyEntry, direction) {
      ...
      //      broken (exception?)
      hashchange: function hashchange(event) {
        var history, hash; // If hashchange listening is explicitly disabled or pushstate is supported
        ...
        hash = path.parseLocation().hash; // If this is a hashchange caused by the back or forward button
  ...
  (function ($, undefined) {
    ...
    $.each(["animation", "transition"], function (i, test) {
      ...
      props[test]["event"] = $.camelCase(props[test]["prefix"] + test + "-" + "end"); // All lower case if not a vendor prop
    ...
    $.support.cssAnimations = props["animation"]["prefix"] !== undefined; // Remove the testElement

    $(testElement).remove(); // Animation complete callback
    ...
    $.fn.animationComplete = function (callback, type, fallbackTime) {
      var timer,
        ...
        callback.apply(this, arguments);
          ...
          animationType = !type || type === "animation" ? "animation" : "transition"; // Make sure selected type is supported by browser
  ...
  })(jQuery); // This plugin is an experiment for abstracting away the touch and mouse
  ...
  })(jQuery, this); // throttled resize event
  ...
  (function ($, window) {
    var win = $(window),
      ...
      "180": true
        ...
        ww,
        wh,
        landscape_threshold; // It seems that some device/browser vendors use window.orientation values 0 and 180 to
    ...
    if ($.support.orientation) {
      ...
      initial_orientation_is_landscape = ww > wh && ww - wh > landscape_threshold; // Now check to see if the current window.orientation is 0 or 180.

      initial_orientation_is_default = portrait_map[window.orientation]; // If the initial orientation is landscape, but window.orientation reports 0 or 180, *OR*
    ...
    $.event.special.orientationchange = $.extend({}, $.event.special.orientationchange, {
      ...
      add: function add(handleObj) {
        ...
        handleObj.handler = function (event) {
          ...
          event.orientation = get_orientation(); // Call the originally-bound event handler and return its result.
    ...
    $.event.special.orientationchange.orientation = get_orientation = function get_orientation() {
      var isPortrait = true,
          elem = document.documentElement; // prefer window orientation to the calculation based on screensize as
  ...
  (function ($, undefined) {
    ...
    base = {
      ...
      rewrite: function rewrite(href, page) {
        var newPath = $.mobile.path.get(href);
        page.find(base.linkSelector).each(function (i, link) {
          var thisAttr = $(link).is("[href]") ? "href" : $(link).is("[src]") ? "src" : "action",
              theLocation = $.mobile.path.parseLocation(),
              thisUrl = $(link).attr(thisAttr); // XXX_jblas: We need to fix this so that it removes the document
  ...
  (function ($, undefined) {
    ...
    }($.widget); // Make sure $.widget still has bridge and extend methods


    $.extend($.widget, originalWidget); // For backcompat remove in 1.5
    ...
    $.widget("mobile.page", {
      ...
      _create: function _create() {
        ...
        this.element.enhanceWithin(); // Dialog widget is deprecated in 1.4 remove this in 1.5
      ...
      _enhance: function _enhance() {
        ...
        this.element.attr("tabindex", "0").addClass("ui-page ui-page-theme-" + this.options.theme); // Manipulation of content os Deprecated as of 1.4 remove in 1.5
      ...
      bindRemove: function bindRemove(callback) {
        var page = this.element; // when dom caching is not enabled or the page is embedded bind to remove the page on hide
      ...
      _setOptions: function _setOptions(o) {
      ...
      setContainerBackground: function setContainerBackground(theme) {
      ...
      keepNativeSelector: function keepNativeSelector() {
        ...
        oldDefault = newDefault === "" ? optionValue : ""; // Concatenate keepNative selectors from all sources where the value has
  ...
  (function ($, undefined) {
    $.widget("mobile.pagecontainer", {
      ...
      _afterContentChange: function _afterContentChange() {
        ...
        this.setLastScrollEnabled = true; // remove any binding that previously existed on the get scroll
      ...
      _recordScroll: function _recordScroll() {
        ...
        if (active) {
          ...
          defaultScroll = this._getDefaultScroll(); // Set active page's lastScroll prop. If the location we're
      ...
      _filterNavigateEvents: function _filterNavigateEvents(e, data) {
        var url;
      ...
      go: function go(steps) {
        ...
        if ($.mobile.hashListeningEnabled) {
          ...
          var activeIndex = $.mobile.navigate.history.activeIndex,
              ...
              direction = steps >= 1 ? "forward" : "back"; //update the history object

          $.mobile.navigate.history.activeIndex = index;
          $.mobile.navigate.history.previousIndex = activeIndex; //change to the new page
      ...
      _handleDestination: function _handleDestination(to) {
        var history; // clean the hash for comparison if it's a url
        ...
        if (to) {
          history = this._getHistory(); // At this point, 'to' can be one of 3 things, a cached page
      ...
      _transitionFromHistory: function _transitionFromHistory(direction, defaultTransition) {
        var history = this._getHistory(),
            entry = direction === "back" ? history.getLast() : history.getActive();

        return entry && entry.transition || defaultTransition;
      },
      _handleDialog: function _handleDialog(changePageOptions, data) {
        var to,
            active,
            activeContent = this.getActivePage(); // If current active page is not a dialog skip the dialog and continue
        ...
        if (activeContent && !activeContent.data("mobile-dialog")) {
          ...
          to = data.pageUrl;
          active = this._getActiveHistory(); // make sure to set the role, transition and reversal
      ...
      _handleNavigate: function _handleNavigate(url, data) {
        ...
        transition = history.stack.length === 0 ? "none" : this._transitionFromHistory(data.direction),
      ...
      _changeContent: function _changeContent(to, opts) {
      ...
      _enhance: function _enhance(content, role) {
      ...
      _include: function _include(page, settings) {
      ...
      _find: function _find(absUrl) {
        // TODO consider supporting a custom callback
        var fileUrl = this._createFileUrl(absUrl),
            dataUrl = this._createDataUrl(absUrl),
            page,
            initialContent = this._getInitialContent(); // Check to see if the page already exists in the DOM.
        // NOTE do _not_ use the :jqmData pseudo selector because parenthesis
        //      are a valid url char and it breaks on the first occurence


        page = this.element.children("[data-" + this._getNs() + "url='" + $.mobile.path.hashToSelector(dataUrl) + "']"); // If we failed to find the page, check to see if the url is a
      ...
      _showLoading: function _showLoading(delay, theme, msg, textonly) {
      ...
      _hideLoading: function _hideLoading() {
        ...
        this._loadMsg = 0; // Hide loading message
      ...
      _showError: function _showError() {
        ...
        this._showLoading(0, $.mobile.pageLoadErrorMessageTheme, $.mobile.pageLoadErrorMessage, true); // hide the error message after a delay
        // TODO configuration


        setTimeout($.proxy(this, "_hideLoading"), 1500);
      },
      _parse: function _parse(html, fileUrl) {
        // TODO consider allowing customization of this method. It's very JQM specific
        var page,
            all = $("<div></div>"); //workaround to allow scripts to execute when included in page divs

        all.get(0).innerHTML = html;
        page = all.find(":jqmData(role='page'), :jqmData(role='dialog')").first(); //if page elem couldn't be found, create one and insert the body element's contents
      ...
      _setLoadedTitle: function _setLoadedTitle(page, html) {
        //page title regexp
      ...
      _createDataUrl: function _createDataUrl(absoluteUrl) {
        return $.mobile.path.convertUrlToDataUrl(absoluteUrl);
      },
      _createFileUrl: function _createFileUrl(absoluteUrl) {
        return $.mobile.path.getFilePath(absoluteUrl);
      },
      _triggerWithDeprecated: function _triggerWithDeprecated(name, data, page) {
        var deprecatedEvent = $.Event("page" + name),
            newEvent = $.Event(this.widgetName + name); // DEPRECATED
      ...
      _loadSuccess: function _loadSuccess(absUrl, triggerData, settings, deferred) {
        var fileUrl = this._createFileUrl(absUrl);
        ...
        return $.proxy(function (html, textStatus, xhr) {
          ...
          pageElemRegex = new RegExp("(<[^>]+\\bdata-" + this._getNs() + "role=[\"']?page[\"']?[^>]*>)"),
              dataUrlRegex = new RegExp("\\bdata-" + this._getNs() + "url=[\"']?([^\"'>]*)[\"']?"); // data-url must be provided for the base tag so resource requests
          // can be directed to the correct url. loading into a temprorary
          // element makes these requests immediately

          if (pageElemRegex.test(html) && RegExp.$1 && dataUrlRegex.test(RegExp.$1) && RegExp.$1) {
            fileUrl = $.mobile.path.getFilePath($("<div>" + RegExp.$1 + "</div>").text()); // We specify that, if a data-url attribute is given on the page div, its value
          ...
          this._setLoadedTitle(content, html); // Add the content reference and xhr to our triggerData.


          triggerData.xhr = xhr;
          triggerData.textStatus = textStatus; // DEPRECATED

          triggerData.page = content;
          triggerData.content = content;
          triggerData.toPage = content; // If the default behavior is prevented, stop here!
          ...
          this._include(content, settings); // Remove loading message.
      ...
      load: function load(url, options) {
        ...
        reloadOptionExtension = options && options.reload === undefined && options.reloadPage !== undefined ? {
        ...
        absUrl = $.mobile.path.makeUrlAbsolute(url, this._findBaseWithDefault()),
            ...
            dataUrl,
            pblEvent,
            triggerData; // If the caller provided data, and we're using "get" request,
        ...
        dataUrl = this._createDataUrl(absUrl);
        content = this._find(absUrl); // If it isn't a reference to the first content and refers to missing
        ...
        this._getBase().reset(); // If the content we are interested in is already in the DOM,
        ...
        if (content.length && !settings.reload) {
          ...
          deferred.resolve(absUrl, settings, content); //if we are reloading the content make sure we update
        ...
        pblEvent = this._triggerWithDeprecated("beforeload", triggerData); // If the default behavior is prevented, stop here!
      ...
      _loadError: function _loadError(absUrl, triggerData, settings, deferred) {
        return $.proxy(function (xhr, textStatus, errorThrown) {
          //set base back to current path
          this._getBase().set($.mobile.path.get()); // Add error info to our triggerData.


          triggerData.xhr = xhr;
          triggerData.textStatus = textStatus;
          triggerData.errorThrown = errorThrown; // Let listeners know the page load failed.

          var plfEvent = this._triggerWithDeprecated("loadfailed", triggerData); // If the default behavior is prevented, stop here!
          // Note that it is the responsibility of the listener/handler
          // that called preventDefault(), to resolve/reject the
          // deferred object within the triggerData.
      ...
      _getTransitionHandler: function _getTransitionHandler(transition) {
        transition = $.mobile._maybeDegradeTransition(transition); //find the transition handler for the specified transition. If there
      ...
      _triggerCssTransitionEvents: function _triggerCssTransitionEvents(to, from, prefix) {
        var samePage = false;
        prefix = prefix || ""; // TODO decide if these events should in fact be triggered on the container
      ...
      _cssTransition: function _cssTransition(to, from, options) {
        ...
        }, this)); // TODO temporary accomodation of argument deferred
      ...
      _removeActiveLinkClass: function _removeActiveLinkClass(force) {
        //clear out the active button state
        $.mobile.removeActiveLinkClass(force);
      },
      _loadUrl: function _loadUrl(to, triggerData, settings) {
        ...
        settings.deferred.done($.proxy(function (url, options, content) {
          isPageTransitioning = false; // store the original absolute url so that it can be provided
      ...
      _triggerPageBeforeChange: function _triggerPageBeforeChange(to, triggerData, settings) {
        var returnEvents;
        ...
        returnEvents = this._triggerWithDeprecated("beforechange", triggerData); // If the default behavior is prevented, stop here!
      ...
      change: function change(to, options) {
        ...
        var settings = $.extend({}, $.mobile.changePage.defaults, options),
            triggerData = {}; // Make sure we have a fromPage.

        settings.fromPage = settings.fromPage || this.activePage; // if the page beforechange default is prevented return early
        ...
        to = triggerData.toPage; // If the caller passed us a url, call loadPage()
      ...
      transition: function transition(toPage, triggerData, settings) {
        var fromPage, url, pageUrl, fileUrl, active, activeIsInitialPage, historyDir, pageTitle, isDialog, alreadyThere, newPageTitle, params, cssTransitionDeferred, beforeTransition; // If we are in the midst of a transition, queue the current request.
        // We'll call changePage() once we're done with the current transition
        // to service the request.
        ...
        triggerData.prevPage = settings.fromPage; // if the (content|page)beforetransition default is prevented return early
        ...
        isPageTransitioning = true; // If we are going to the first-page of the application, we need to make
        ...
        url = settings.dataUrl && $.mobile.path.convertUrlToDataUrl(settings.dataUrl) || toPage.jqmData("url"); // The pageUrl var is usually the same as url, except when url is obscured
        ...
        isDialog = (settings.role === "dialog" || toPage.jqmData("role") === "dialog") && toPage.jqmData("dialog") !== true; // By default, we prevent changePage requests when the fromPage and toPage
        ...
        if (fromPage && fromPage[0] === toPage[0] && !settings.allowSamePageTransition) {
          ...
          this._triggerWithDeprecated("change", triggerData); // Even if there is no page change to be done, we should keep the
        ...
        } catch (e) {} // Record whether we are at a place in history where a dialog used to be -
        // if so, do not add a new history entry and do not change the hash either


        alreadyThere = false; // If we're displaying the page as a dialog, we don't want the url
        ...
        if (isDialog && active) {
          ...
          url = active.url || ""; // account for absolute urls instead of just relative urls use as hashes
        ...
        settings.transition = settings.transition || (historyDir && !activeIsInitialPage ? active.transition : undefined) || (isDialog ? $.mobile.defaultDialogTransition : $.mobile.defaultPageTransition); //add page to history stack if it's not back or forward
        ...
        document.title = pageTitle; //set "toPage" as activePage deprecated in 1.4 remove in 1.5

        $.mobile.activePage = toPage; //new way to handle activePage
        ...
        cssTransitionDeferred.done($.proxy(function (name, reverse, $to, $from, alreadyFocused) {
          $.mobile.removeActiveLinkClass(); //if there's a duplicateCachedPage, remove it from the DOM now that it's hidden
    ...
    $.mobile.navreadyDeferred = $.Deferred(); //these variables make all page containers use the same queue and only navigate one at a time
  ...
  (function ($, undefined) {
    ...
    pageIsFullyLoaded = function pageIsFullyLoaded() {
    ...
    $.mobile.loadPage = function (url, opts) {
      var container;
      opts = opts || {};
      container = opts.pageContainer || $.mobile.pageContainer; // create the deferred that will be supplied to loadPage callers
      // and resolved by the content widget's load method

      opts.deferred = $.Deferred(); // Preferring to allow exceptions for uninitialized opts.pageContainer
      // widgets so we know if we need to force init here for users

      container.pagecontainer("load", url, opts); // provide the deferred
    ...
    $.mobile.back = function () {
      var nav = window.navigator; // if the setting is on and the navigator object is
    ...
    $.mobile._registerInternalEvents = function () {
      var getAjaxFormData = function getAjaxFormData($form, calculateOnly) {
        var url,
            ret = true,
            formData,
            vclickedName,
        ...
        !$form.jqmHijackable().length || $form.attr("target")) {
          return false;
        }

        url = $lastVClicked && $lastVClicked.attr("formaction") || $form.attr("action");
        method = ($form.attr("method") || "get").toLowerCase(); // If no action is specified, browsers default to using the
        ...
        if (!url) {
          ...
          url = $.mobile.getClosestBaseUrl($form); // NOTE: If the method is "get", we need to strip off the query string
      ...
      $.mobile.document.bind("vclick", function (event) {
        var $btn,
            btnEls,
            target = event.target,
            needClosest = false; // if this isn't a left click we don't care. Its important to note
        ...
        $lastVClicked = $(target); // Try to find a target element to which the active class will be applied
        ...
        $btn = $(target); // If the outer element wasn't found by the our heuristics, use .closest()

        if (needClosest) {
          $btn = $btn.closest(".ui-btn");
        }

        if ($btn.length > 0 && !$btn.hasClass("ui-state-disabled" || // DEPRECATED as of 1.4.0 - remove after 1.4.0 release
      ...
      $.mobile.document.bind("click", function (event) {
        ...
        httpCleanup = function httpCleanup() {
          window.setTimeout(function () {
            ...
            transition,
            reverse,
            role; // If a button was clicked, clean up the active class added by vclick above
        ...
        baseUrl = $.mobile.getClosestBaseUrl($link); //get href, if defined, otherwise default to empty hash

        href = $.mobile.path.makeUrlAbsolute($link.attr("href") || "#", baseUrl); //if ajax is disabled, exit early

        if (!$.mobile.ajaxEnabled && !$.mobile.path.isEmbeddedPage(href)) {
          httpCleanup(); //use default click handling
        ...
        useDefaultUrlHandling = $link.is("[rel='external']") || $link.is(":jqmData(ajax='false')") || $link.is("[target]"); // Some embedded browsers, like the web view in Phone Gap, allow cross-domain XHR
        ...
        reverse = $link.jqmData("direction") === "reverse" || // deprecated - remove by 1.0
  ...
  (function ($, window, undefined) {
    ...
    $.extend($.mobile.Transition.prototype, {
      ...
      init: function init(name, reverse, $to, $from) {
        $.extend(this, {
          name: name,
          reverse: reverse,
          $to: $to,
      ...
      beforeDoneIn: function beforeDoneIn() {},
      beforeDoneOut: function beforeDoneOut() {},
      beforeStartOut: function beforeStartOut() {},
      doneIn: function doneIn() {
        this.beforeDoneIn();
        this.$to.removeClass("out in reverse " + this.name).height("");
        this.toggleViewportClass(); // In some browsers (iOS5), 3D transitions block the ability to scroll to the desired location during transition
      ...
      doneOut: function doneOut(screenHeight, reverseClass, none, preventFocus) {
        this.beforeDoneOut();
        this.startIn(screenHeight, reverseClass, none, preventFocus);
      },
      hideIn: function hideIn(callback) {
      ...
      scrollPage: function scrollPage() {
        ...
        $.event.special.scrollstart.enabled = false; //if we are hiding the url bar or the page was previously scrolled scroll to hide or return to position
      ...
      startIn: function startIn(screenHeight, reverseClass, none, preventFocus) {
        this.hideIn(function () {
          this.$to.addClass($.mobile.activePageClass + this.toPreClass); // Send focus to page as it is now display: block
      ...
      startOut: function startOut(screenHeight, reverseClass, none) {
  ...
  (function ($) {
    ...
    $.extend($.mobile.SerialTransition.prototype, $.mobile.Transition.prototype, {
      ...
      beforeStartOut: function beforeStartOut(screenHeight, reverseClass, none) {
  ...
  (function ($) {
    ...
    var defaultGetMaxScrollForTransition = function defaultGetMaxScrollForTransition() {
    ...
    $.mobile.transitionFallbacks = {}; // If transition is defined, check if css 3D transforms are supported, and if not, if a fallback is specified
  ...
  */
  ...
  (function ($, window, undefined) {
    ...
    $.mobile.transitionHandlers.slide = $.mobile.transitionHandlers.simultaneous; // Set the slide transitions's fallback to "fade"
  ...
  */
  ...
  (function ($, undefined) {
    ...
    $.mobile.page.prototype.options.degradeInputs = $.mobile.degradeInputs; // Auto self-init widgets

    $.mobile.degradeInputsWithin = function (target) {
      target = $(target); // Degrade inputs to avoid poorly implemented native functionality
      ...
      target.find("input").not($.mobile.page.prototype.keepNativeSelector()).each(function () {
        ...
        if ($.mobile.degradeInputs[type]) {
          html = $("<div>").html(element.clone()).html(); // In IE browsers, the type sometimes doesn't exist in the cloned markup, so we replace the closing tag instead
  ...
  (function ($, window, undefined) {
    $.widget("mobile.page", $.mobile.page, {
      ...
      _enhance: function _enhance() {
        this._super(); // Class the markup for dialog styling and wrap interior
      ...
      _setCloseBtn: function _setCloseBtn(location, text) {
        var dst,
            btn = this._headerCloseButton; // Sanitize value
  ...
  (function ($, window, undefined) {
    $.widget("mobile.dialog", {
      ...
      _handleVClickSubmit: function _handleVClickSubmit(event) {
        var attrs,
      ...
      _create: function _create() {
        var elem = this.element,
            opts = this.options; // Class the markup for dialog styling and wrap interior
      ...
      close: function close() {
        ...
        if (this._isCloseable) {
          this._isCloseable = false; // If the hash listening is enabled and there is at least one preceding history
  ...
  (function ($, undefined) {
    ...
    iconposClass = function iconposClass(iconpos) {
    ...
    $.widget("mobile.collapsible", {
      ...
      _getOptions: function _getOptions(options) {
        var key,
            ...
            accordionWidget = this._ui.accordionWidget; // Copy options
      ...
      _themeClassFromOption: function _themeClassFromOption(prefix, value) {
        return value ? value === "none" ? "" : prefix + value : "";
      },
      _enhance: function _enhance(elem, ui) {
        var iconclass,
        ...
        ui.originalHeading = elem.children(this.options.heading).first(), ui.content = elem.wrapInner("<div " + "class='ui-collapsible-content " + contentThemeClass + "'></div>").children(".ui-collapsible-content"), ui.heading = ui.originalHeading; // Replace collapsibleHeading if it's a legend
        ...
        ui.anchor = ui.heading.detach() //modify markup & attributes
      ...
      _applyOptions: function _applyOptions(options) {
        var isCollapsed,
            ...
            anchor = ui.anchor,
            status = ui.status,
            opts = this._getOptions(options); // First and foremost we need to make sure the collapsible is in the proper
        ...
        isCollapsed = elem.hasClass("ui-collapsible-collapsed"); // We only need to apply the cue text for the current state right away.
        ...
        opts.collapsedIcon !== undefined ? opts.collapsedIcon !== false : // Otherwise consult the existing option value
        currentOpts.collapsedIcon !== false; // If any icon-related options have changed, make sure the new icon
        ...
        if (!(opts.iconpos === undefined && opts.collapsedIcon === undefined && opts.expandedIcon === undefined)) {
          ...
          anchor.removeClass([iconposClass(currentOpts.iconpos)].concat(currentOpts.expandedIcon ? ["ui-icon-" + currentOpts.expandedIcon] : []).concat(currentOpts.collapsedIcon ? ["ui-icon-" + currentOpts.collapsedIcon] : []).join(" ")); // Add new classes if an icon is supposed to be present
      ...
      _handleExpandCollapse: function _handleExpandCollapse(isCollapse) {
        var opts = this._renderedOptions,
            ui = this._ui;
        ui.status.text(isCollapse ? opts.expandCueText : opts.collapseCueText);
        ui.heading.toggleClass("ui-collapsible-heading-collapsed", isCollapse).find("a").first().toggleClass("ui-icon-" + opts.expandedIcon, !isCollapse) // logic or cause same icon for expanded/collapsed state would remove the ui-icon-class
        .toggleClass("ui-icon-" + opts.collapsedIcon, isCollapse || opts.expandedIcon === opts.collapsedIcon).removeClass($.mobile.activeBtnClass);
  ...
  (function ($, undefined) {
    ...
    $.mobile.behaviors.addFirstLastClasses = {
      _getVisibles: function _getVisibles($els, create) {
        var visibles;
      ...
      _addFirstLastClasses: function _addFirstLastClasses($els, $visibles, create) {
      ...
      _removeFirstLastClasses: function _removeFirstLastClasses($els) {
        $els.removeClass("ui-first-child ui-last-child");
      }
    };
  })(jQuery);
  ...
  (function ($, undefined) {
    ...
    $.widget("mobile.collapsibleset", $.extend({
      ...
      _handleCollapsibleExpand: function _handleCollapsibleExpand(event) {
      ...
      _init: function _init() {
        this._refresh(true); // Because the corners are handled by the collapsible itself and the default state is collapsed
      ...
      _refresh: function _refresh(create) {
    ...
    $.widget("mobile.navbar", {
      ...
      _create: function _create() {
        ...
        /* event */
        {
          ...
          if (!(activeBtn.hasClass("ui-state-disabled") || // DEPRECATED as of 1.4.0 - remove after 1.4.0 release
          // only ui-state-disabled should be present thereafter
          activeBtn.hasClass("ui-disabled") || activeBtn.hasClass($.mobile.activeBtnClass))) {
            $navbtns.removeClass($.mobile.activeBtnClass);
            activeBtn.addClass($.mobile.activeBtnClass); // The code below is a workaround to fix #1181
  ...
  (function ($, undefined) {
    ...
    $.widget("mobile.listview", $.extend({
      ...
      // TODO: Remove in 1.5
      _findFirstElementByTagName: function _findFirstElementByTagName(ele, nextProp, lcName, ucName) {
        var dict = {};
      ...
      // TODO: Remove in 1.5
      _addThumbClasses: function _addThumbClasses(containers) {
        var i,
            img,
      ...
      _getChildrenByTagName: function _getChildrenByTagName(ele, lcName, ucName) {
        var results = [],
            dict = {};
        dict[lcName] = dict[ucName] = true;
        ele = ele.firstChild;
      ...
      refresh: function refresh(create) {
        var buttonClass,
            pos,
            numli,
            item,
        ...
        for (pos = 0, numli = li.length; pos < numli; pos++) {
          ...
          if (create || item[0].className.search(/\bui-li-static\b|\bui-li-divider\b/) < 0) {
            ...
            if (a.length && a[0].className.search(/\bui-btn\b/) < 0 && !isDivider) {
              ...
              icon = itemIcon === false ? false : itemIcon || o.icon; // TODO: Remove in 1.5 together with links.js (links.js / .ui-link deprecated in 1.4)
              ...
              if (a.length > 1) {
                ...
                last.attr("title", $.trim(last.getEncodedText())).addClass(altButtonClass).empty(); // Reduce to the first anchor, because only the first gets the buttonClass
  ...
  (function ($, undefined) {
    ...
    $.widget("mobile.listview", $.mobile.listview, {
      ...
      _beforeListviewRefresh: function _beforeListviewRefresh() {
  ...
  (function ($, undefined) {
    ...
    $.widget("mobile.listview", $.mobile.listview, {
      ...
      _afterListviewRefresh: function _afterListviewRefresh() {
        var items,
            idx,
            item,
            hideDivider = true;
  ...
  (function ($, undefined) {
    $.mobile.behaviors.formReset = {
      _handleFormReset: function _handleFormReset() {
  ...
  */
    ...
    $.widget("mobile.checkboxradio", $.extend({
      ...
      _create: function _create() {
        var input = this.element,
            o = this.options,
            inheritAttr = function inheritAttr(input, dataAttr) {
        ...
        o.iconpos = inheritAttr(input, "iconpos") || label.element.attr("data-" + $.mobile.ns + "iconpos") || o.iconpos, // Establish options
      ...
      _findLabel: function _findLabel() {
        ...
        if (labelsList && labelsList.length > 0) {
          ...
          isParent = parentLabel.length > 0; // NOTE: Windows Phone could not find the label through a selector
          // filter works though.
      ...
      _handleLabelVMouseOver: function _handleLabelVMouseOver(event) {
        if (this.label.parent().hasClass("ui-state-disabled")) {
          event.stopPropagation();
        }
      },
      _handleLabelVClick: function _handleLabelVClick(event) {
        var input = this.element;
        ...
        input.prop("checked", this.inputtype === "radio" && true || !input.prop("checked")); // trigger click handler's bound directly to the input as a substitute for
      ...
      _getInputSet: function _getInputSet() {
        ...
        radios = this.element; // Only start running selectors if this is an attached radio button with a name

        if (name && this.inputtype === "radio" && doc) {
          selector = "input[type='radio'][name='" + escapeId(name) + "']"; // If we're inside a form

          if (form) {
            formId = form.getAttribute("id"); // If the form has an ID, collect radios scattered throught the document which
            ...
            }).add(radios); // If we're outside a form
          } else {
      ...
      _updateAll: function _updateAll(changeTriggered) {
        var self = this;
      ...
      _hasIcon: function _hasIcon() {
        var controlgroup,
            controlgroupWidget,
            controlgroupConstructor = $.mobile.controlgroup; // If the controlgroup widget is defined ...

        if (controlgroupConstructor) {
          controlgroup = this.element.closest(":mobile-controlgroup," + controlgroupConstructor.prototype.initSelector); // ... and the checkbox is in a controlgroup ...

          if (controlgroup.length > 0) {
            // ... look for a controlgroup widget instance, and ...
            controlgroupWidget = $.data(controlgroup[0], "mobile-controlgroup"); // ... if found, decide based on the option value, ...

            return (controlgroupWidget ? controlgroupWidget.options.type : // ... otherwise decide based on the "type" data attribute.
  ...
  (function ($, undefined) {
    $.widget("mobile.button", {
      ...
      _getIconClasses: function _getIconClasses(options) {
        return options.icon ? "ui-icon-" + options.icon + (options.iconshadow ? " ui-shadow-icon" : "") +
        /* TODO: Deprecated in 1.4, remove in 1.5. */
      ...
      _setOptions: function _setOptions(options) {
        ...
        if (options.icon !== undefined || options.iconshadow !== undefined ||
        /* TODO: Deprecated in 1.4, remove in 1.5. */
    ...
    $.widget("mobile.slider", $.extend({
      ...
      _create: function _create() {
        ...
        if (isToggleSwitch) {
          ...
          domSlider.appendChild(wrapper); // slider.wrapInner( "<div class='ui-slider-inneroffset'></div>" );
        ...
        slider.bind("vmousedown", $.proxy(this._sliderVMouseDown, this)).bind("vclick", false); // We have to instantiate a new function object for the unbind to work properly
        ...
        slider.insertAfter(control); // wrap in a div for styling purposes
      ...
      _controlChange: function _controlChange(event) {
      ...
      _sliderVMouseDown: function _sliderVMouseDown(event) {
      ...
      _preventDocumentDrag: function _preventDocumentDrag(event) {
        // NOTE: we don't do this in refresh because we still want to
        //       support programmatic alteration of disabled inputs
        ...
        if (this.dragging && !this.options.disabled) {
          ...
          this.refresh(event); // only after refresh() you can calculate this.userModified
      ...
      refresh: function refresh(val, isfromControl, preventInputUpdate) {
        ...
        if (_typeof(val) === "object") {
          data = val; // a slight tolerance helped get to the ends of the slider
        ...
        newval = percent / 100 * (max - min) + min; //from jQuery UI slider, the following source will round to the nearest step
        ...
        percentPerStep = 100 / ((max - min) / step); // Since JavaScript has problems with large floats, round
        ...
        if (!preventInputUpdate) {
          valueChanged = false; // update control"s value
      ...
      _setHighlight: function _setHighlight(value) {
        value = !!value;
      ...
      _setTheme: function _setTheme(value) {
      ...
      _setCorners: function _setCorners(value) {
      ...
      _setDisabled: function _setDisabled(value) {
        value = !!value;
  ...
  (function ($, undefined) {
    $.widget("mobile.flipswitch", $.extend({
      ...
      _create: function _create() {
        ...
        this._handleFormReset(); // Transfer tabindex to "on" element and make input unfocusable
  ...
  (function ($, undefined) {
    $.widget("mobile.rangeslider", $.extend({
      ...
      _handleReset: function _handleReset() {
        var self = this; //we must wait for the stack to unwind before updateing other wise sliders will not have updated yet

        setTimeout(function () {
          self._updateHighlight();
        }, 0);
      },
      _dragFirstHandle: function _dragFirstHandle(event) {
      ...
      _slidedrag: function _slidedrag(event) {
        var first = $(event.target).is(this._inputFirst),
            otherSlider = first ? this._inputLast : this._inputFirst;
        this._sliderTarget = false; //if the drag was initiated on an extreme and the other handle is focused send the events to
        //the closest handle
      ...
      _slidestop: function _slidestop(event) {
        var first = $(event.target).is(this._inputFirst);
        this._proxy = false; //this stops dragging of the handle and brings the active track to the front
      ...
      _slidebeforestart: function _slidebeforestart(event) {
        this._sliderTarget = false; //if the track is the target remember this and the original value
  ...
  (function ($, undefined) {
    $.widget("mobile.textinput", $.mobile.textinput, {
      ...
      _clearBtnClick: function _clearBtnClick(event) {
  ...
  (function ($, undefined) {
    $.widget("mobile.textinput", $.mobile.textinput, {
      ...
      _handleShow: function _handleShow(event) {
      ...
      keyupTimeout: null,
      _prepareHeightUpdate: function _prepareHeightUpdate(delay) {
      ...
      _updateHeight: function _updateHeight() {
        var paddingTop,
            ...
            height,
            scrollTop = this.window.scrollTop();
        this.keyupTimeout = 0; // IE8 textareas have the onpage property - others do not
        ...
        height = scrollHeight + borderHeight + 15; // Issue 6179: Padding is not included in scrollHeight and
    ...
    $.widget("mobile.selectmenu", $.extend({
      ...
      _preExtension: function _preExtension() {
        var inline = this.options.inline || this.element.jqmData("inline"),
            ...
            classes = ""; // TODO: Post 1.1--once we have time to test thoroughly--any classes manually applied to the original element should be carried over to the enhanced element, with an `-enhanced` suffix. See https://github.com/jquery/jquery-mobile/issues/3577
      ...
      _create: function _create() {
        ...
        var self = this,
            ...
            button = this.button.insertBefore(this.select).attr("id", this.buttonId).addClass("ui-btn" + (options.icon ? " ui-icon-" + options.icon + " ui-btn-icon-" + iconpos + (options.iconshadow ? " ui-shadow-icon" : "") : "") + (
        /* TODO: Remove in 1.5. */
        options.theme ? " ui-btn-" + options.theme : "") + (options.corners ? " ui-corner-all" : "") + (options.shadow ? " ui-shadow" : ""));
        this.setButtonText(); // Opera does not properly support opacity on select elements
      ...
      setButtonCount: function setButtonCount() {
        var selected = this.selected(); // multiple count inside button
  ...
  (function ($, undefined) {
    ...
    $.widget("mobile.popup", {
      ...
      _handleDocumentVmousedown: function _handleDocumentVmousedown(theEvent) {
      ...
      _create: function _create() {
        var theElement = this.element,
            ...
            currentOptions = this.options; // We need to adjust the history option to be false if there's no AJAX nav.
        ...
        this._setTolerance(currentOptions.tolerance)._ui.focusElement = this._ui.container; // Event handlers
      ...
      _enhance: function _enhance(theElement, myId) {
      ...
      _eatEventAndClose: function _eatEventAndClose(theEvent) {
      ...
      _handleWindowKeyUp: function _handleWindowKeyUp(theEvent) {
      ...
      _handleDocumentFocusIn: function _handleDocumentFocusIn(theEvent) {
        var target,
      ...
      _applyTransition: function _applyTransition(value) {
        if (value) {
      ...
      _setOptions: function _setOptions(newOptions) {
        var currentOptions = this.options,
      ...
      _setTolerance: function _setTolerance(value) {
        var tol = {
          t: 30,
          r: 15,
          b: 30,
      ...
      _clampPopupWidth: function _clampPopupWidth(infoOnly) {
        var menuSize,
      ...
      _calculateFinalLocation: function _calculateFinalLocation(desired, clampInfo) {
        var returnValue,
            rectangle = clampInfo.rc,
            menuSize = clampInfo.menuSize; // Center the menu over the desired coordinates, while not going outside
        ...
        returnValue.top = Math.max(0, returnValue.top); // If the height of the menu is smaller than the height of the document
      ...
      _placementCoords: function _placementCoords(desired) {
        return this._calculateFinalLocation(desired, this._clampPopupWidth());
      },
      _createPrerequisites: function _createPrerequisites(screenPrerequisite, containerPrerequisite, whenDone) {
        var prerequisites,
            self = this; // It is important to maintain both the local variable prerequisites and
      ...
      _animate: function _animate(args) {
      ...
      _desiredCoords: function _desiredCoords(openOptions) {
        var offset,
            dst = null,
            windowCoordinates = getWindowCoordinates(this.window),
            x = openOptions.x,
            y = openOptions.y,
            pTo = openOptions.positionTo; // Establish which element will serve as the reference
      ...
      _reposition: function _reposition(openOptions) {
      ...
      reposition: function reposition(openOptions) {
        if (this._isOpen) {
          this._reposition(openOptions);
        }
      },
      _safelyBlur: function _safelyBlur(currentElement) {
      ...
      _openPrerequisitesComplete: function _openPrerequisitesComplete() {
        ...
        this._resizeScreen(); // Check to see if currElement is not a child of the container.  If it's not, blur
      ...
      _open: function _open(options) {
        ...
        androidBlacklist = function () {
          ...
          wkmatch = ua.match(/AppleWebKit\/([0-9\.]+)/),
              ...
              chromematch = ua.indexOf("Chrome") > -1; // Platform is Android, WebKit version is greater than 534.13 ( Android 3.2.1 ) and not Chrome.
        ...
        this._ui.container.removeClass("ui-popup-truncate"); // Give applications a chance to modify the contents of the container before it appears
      ...
      _closePrerequisitesDone: function _closePrerequisitesDone() {
        var container = this._ui.container,
            id = this.element.attr("id"); // remove the global mutex for popups
      ...
      _close: function _close(immediate) {
        this._ui.container.removeClass("ui-popup-active");

        this._page.removeClass("ui-popup-open");

        this._isOpen = false; // Count down to triggering "popupafterclose" - we have two prerequisites:
      ...
      _unenhance: function _unenhance() {
        ...
        .detach().insertAfter(this._ui.placeholder).removeClass("ui-popup ui-overlay-shadow ui-corner-all ui-body-inherit");
      ...
      _closePopup: function _closePopup(theEvent, data) {
        var parsedDst,
            toUrl,
            currentOptions = this.options,
            immediate = false;
      ...
      open: function open(options) {
        var url,
            ...
            urlHistory,
            self = this,
            currentOptions = this.options; // make sure open is idempotent
        ...
        this._scrollTop = this.window.scrollTop(); // if history alteration is disabled close on navigate events
        // and leave the url as is

        if (!currentOptions.history) {
          self._open(options);

          self._bindContainerClose(); // When histoy is disabled we have to grab the data-rel
  ...
  */
  ...
  (function ($, undefined) {
    var unfocusableItemSelector = ".ui-disabled,.ui-state-disabled,.ui-li-divider,.ui-screen-hidden,:jqmData(role='placeholder')",
        goToAdjacentItem = function goToAdjacentItem(item, target, direction) {
      var adjacent = item[direction + "All"]().not(unfocusableItemSelector).first(); // if there's a previous option, focus it
    ...
    $.widget("mobile.selectmenu", $.mobile.selectmenu, {
      _create: function _create() {
        var o = this.options; // Custom selects cannot exist inside popups, so revert the "nativeMenu"
      ...
      _handleButtonVclickKeydown: function _handleButtonVclickKeydown(event) {
        ...
        if (event.type === "vclick" || event.keyCode && (event.keyCode === $.mobile.keyCode.ENTER || event.keyCode === $.mobile.keyCode.SPACE)) {
          ...
          this.isOpen = true; // Do not prevent default, so the navigation may have a chance to actually open the chosen format
        }
      },
      _handleListFocus: function _handleListFocus(e) {
      ...
      _handleListKeydown: function _handleListKeydown(event) {
        var target = $(event.target),
            li = target.closest("li"); // switch logic based on which key was pressed
      ...
      _handleMenuPageHide: function _handleMenuPageHide() {
        ...
        this._delayedTrigger(); // TODO centralize page removal binding / handling in the page plugin.
      ...
      _handleListItemClick: function _handleListItemClick(event) {
        var listItem = $(event.target).closest("li"),
            // Index of option tag to be selected
        oldIndex = this.select[0].selectedIndex,
            newIndex = $.mobile.getAttribute(listItem, "option-index"),
            option = this._selectOptions().eq(newIndex)[0]; // Toggle selected status on the tag for multi selects


        option.selected = this.isMultiple ? !option.selected : true; // Toggle checkbox class for multiple selects
      ...
      _isRebuildRequired: function _isRebuildRequired() {
        var list = this.list.find("li"),
            options = this._selectOptions().not(".ui-screen-hidden"); // TODO exceedingly naive method to determine difference
      ...
      refresh: function refresh(force) {
        ...
        self.list.find("li:not(.ui-li-divider)").find("a").removeClass($.mobile.activeBtnClass).end().attr("aria-selected", false).each(function (i) {
          ...
          if ($.inArray(i, indices) > -1) {
            // Aria selected attr
            item.attr("aria-selected", true); // Multiple selects: add the "on" checkbox state to the icon
      ...
      close: function close() {
        ...
        self._focusButton(); // allow the dialog to be closed again
      ...
      _decideFormat: function _decideFormat() {
        ...
        if (menuHeight > screenHeight - 80 || !$.support.scrollTop) {
          ...
          self.menuPageClose = self.menuPage.find(".ui-header a"); // prevent the parent page from being removed from the DOM,
          // otherwise the results of selecting a list item in the dialog
          // fall into a black hole

          self.thisPage.unbind("pagehide.remove"); //for WebOS/Opera Mini (set lastscroll using button offset)
      ...
      _buildList: function _buildList() {
        ...
        for (i = 0; i < numOptions; i++, isPlaceholderItem = false) {
          option = $options[i];
          $option = $(option); // Do not create options based on ui-screen-hidden select options
          ...
          classes = []; // Although using .text() here raises the risk that, when we later paste this into the
          ...
          anchor.appendChild(document.createTextNode(text)); // Are we inside an optgroup?
          ...
          if (needPlaceholder && (!option.getAttribute("value") || text.length === 0 || $option.jqmData("placeholder"))) {
            ...
            isPlaceholderItem = true; // If we have identified a placeholder, record the fact that it was
        ...
        self.list[0].appendChild(fragment); // Hide header if it's not a multiselect and there's no placeholder
      ...
      _destroy: function _destroy() {
        if (!this.options.nativeMenu) {
          this.close(); // Restore the tabindex attribute to its original value
  ...
  (function ($, undefined) {
    ...
    var reverseBoolOptionMap = {
      ...
      "ui-mini": "mini"
        ...
        getAttrFixed = function getAttrFixed() {
      var ret = $.mobile.getAttribute.apply(this, arguments);
      return ret == null ? undefined : ret;
    },
        capitalLettersRE = /[A-Z]/g; // optionsToClasses:
    ...
    function optionsToClasses(options, existingClasses) {
      var classes = existingClasses ? existingClasses : []; // Add classes to the array - first ui-btn

      classes.push("ui-btn"); // If there is a theme
    ...
    function classNameToOptions(classes) {
      var idx,
        ...
        mini: false
          ...
          unknownClasses = [];
      classes = classes.split(" "); // Loop over the classes

      for (idx = 0; idx < classes.length; idx++) {
        // Assume it's an unrecognized class
        unknownClass = true; // Recognize boolean options from the presence of classes

        map = reverseBoolOptionMap[classes[idx]];

        if (map !== undefined) {
          unknownClass = false;
          o[map] = true; // Recognize the presence of an icon and establish the icon position
        } else if (classes[idx].indexOf("ui-btn-icon-") === 0) {
          unknownClass = false;
          noIcon = false;
          o.iconpos = classes[idx].substring(12); // Establish which icon is present
        } else if (classes[idx].indexOf("ui-icon-") === 0) {
          unknownClass = false;
          o.icon = classes[idx].substring(8); // Establish the theme - this recognizes one-letter theme swatch names
        } else if (classes[idx].indexOf("ui-btn-") === 0 && classes[idx].length === 8) {
          unknownClass = false;
          o.theme = classes[idx].substring(7); // Recognize that this element has already been buttonMarkup-enhanced
    ...
    $.fn.buttonMarkup = function (options, overwriteClasses) {
      ...
      for (idx = 0; idx < this.length; idx++) {
        el = this[idx];
        data = overwriteClasses ? // Assume this element is not enhanced and ignore its classes
        {
          alreadyEnhanced: false,
          unknownClasses: []
        ...
        retrievedOptions = $.extend({}, // If the element already has the class ui-btn, then we assume that
        // it has passed through buttonMarkup before - otherwise, the options
        // returned by classNameToOptions do not correctly reflect the state of
        // the element
        data.alreadyEnhanced ? data.options : {}, // Finally, apply the options passed in
        options); // If this is the first call on this element, retrieve remaining options
        ...
        el.className = optionsToClasses( // Merge all the options and apply them as classes
  ...
  (function ($, undefined) {
    $.widget("mobile.controlgroup", $.extend({
      ...
      _create: function _create() {
        var elem = this.element,
            opts = this.options,
            keepNative = $.mobile.page.prototype.keepNativeSelector(); // Run buttonmarkup
      ...
      _themeClassFromOption: function _themeClassFromOption(value) {
      ...
      _setOptions: function _setOptions(options) {
        var callRefresh,
            returnValue,
            elem = this.element; // Must have one of horizontal or vertical
  ...
  (function ($, undefined) {
    $.widget("mobile.toolbar", {
      ...
      _addHeaderButtonClasses: function _addHeaderButtonClasses() {
        var headerAnchors = this.element.children("a, button"); // Do not mistake a back button for a left toolbar button

        this.leftbtn = headerAnchors.hasClass("ui-btn-left") && !headerAnchors.hasClass("ui-toolbar-back-btn");
        this.rightbtn = headerAnchors.hasClass("ui-btn-right"); // Filter out right buttons and back buttons
      ...
      _updateBackButton: function _updateBackButton() {
        var backButton,
            options = this.options,
            theme = options.backBtnTheme || options.theme; // Retrieve the back button or create a new, empty one

        backButton = this._backButton = this._backButton || {}; // We add a back button only if the option to do so is on

        if (this.options.addBackBtn && // This must also be a header toolbar
        this.role === "header" && // There must be multiple pages in the DOM
        $(".ui-page").length > 1 && (this.page ? // If the toolbar is internal the page's URL must differ from the hash
        this.page[0].getAttribute("data-" + $.mobile.ns + "url") !== $.mobile.path.stripHash(location.hash) : // Otherwise, if the toolbar is external there must be at least one
        // history item to which one can go back
        $.mobile.navigate && $.mobile.navigate.history && $.mobile.navigate.history.activeIndex > 0) && // The toolbar does not have a left button
        !this.leftbtn) {
  ...
  (function ($, undefined) {
    $.widget("mobile.toolbar", $.mobile.toolbar, {
      options: {
        ...
        supportBlacklist: function supportBlacklist() {
      ...
      _bindPageEvents: function _bindPageEvents() {
        var page = !!this.page ? this.element.closest(".ui-page") : this.document; //page event bindings
      ...
      _handlePageBeforeHide: function _handlePageBeforeHide(e, ui) {
        var o = this.options,
            thisFooter,
            thisHeader,
            nextFooter,
      ...
      updatePagePadding: function updatePagePadding(tbPage) {
        var $el = this.element,
            header = this.role === "header",
            pos = parseFloat($el.css(header ? "top" : "bottom")); // This behavior only applies to "fixed", not "fullscreen"
      ...
      _useTransition: function _useTransition(notransition) {
        var $win = this.window,
            $el = this.element,
      ...
      show: function show(notransition) {
      ...
      hide: function hide(notransition) {
        var hideClass = "ui-fixed-hidden",
            $el = this.element,
      ...
      _bindToggleHandlers: function _bindToggleHandlers() {
        var self = this,
            ...
            delayHide,
            isVisible = true,
            page = !!this.page ? this.page : $(".ui-page"); // tap toggle
        ...
        page.bind("vclick", function (e) {
          ...
          if (screen.width < 1025 && $(e.target).is(o.hideDuringFocus) && !$(e.target).closest(".ui-header-fixed, .ui-footer-fixed").length) {
            ...
            if (e.type === "focusout" && !isVisible) {
              isVisible = true; //wait for the stack to unwind and see if we have jumped to another input
    ...
    $.widget("mobile.toolbar", $.mobile.toolbar, {
      ...
      _workarounds: function _workarounds() {
        ...
        wkmatch = ua.match(/AppleWebKit\/([0-9]+)/),
            wkversion = !!wkmatch && wkmatch[1],
            os = null,
            self = this; //set the os we are working in if it dosent match one with workarounds return
      ...
      _bindScrollWorkaround: function _bindScrollWorkaround() {
        var self = this; //bind to scrollstop and check if the toolbars are correctly positioned

        this._on(this.window, {
          scrollstop: function scrollstop() {
            var viewportOffset = self._viewportOffset(); //check if the header is visible and if its in the right place
      ...
      _triggerRedraw: function _triggerRedraw() {
        var paddingBottom = parseFloat($(".ui-page-active").css("padding-bottom")); //trigger page redraw to fix incorrectly positioned fixed elements
  ...
  (function ($, undefined) {
    ...
    $.widget("mobile.popup", $.mobile.popup, {
      ...
      _tryAnArrow: function _tryAnArrow(p, dir, desired, s, best) {
        var result,
            r,
            diff,
            desiredForArrow = {},
            tip = {}; // If the arrow has no wiggle room along the edge of the popup, it cannot
      ...
      _getPlacementState: function _getPlacementState(clamp) {
        var offset,
            gdOffset,
            ar = this._ui.arrow,
            state = {
      ...
      _placementCoords: function _placementCoords(desired) {
        ...
        }, this)); // Could not place the arrow along any of the edges - behave as if showing
        ...
        ar.ct.removeClass("ui-popup-arrow-l ui-popup-arrow-t ui-popup-arrow-r ui-popup-arrow-b").addClass("ui-popup-arrow-" + best.dir).removeAttr("style").css(best.posProp, best.posVal).show(); // Do not move/size the background div on IE, because we use the arrow div for background as well.
      ...
      _setOptions: function _setOptions(opts) {
        ...
        if (opts.arrow !== undefined) {
          if (!ar && opts.arrow) {
            this._ui.arrow = this._addArrow(); // Important to return here so we don't set the same options all over
            // again below.
  ...
  (function ($, undefined) {
    $.widget("mobile.panel", {
      ...
      _create: function _create() {
        var el = this.element,
            parentPage = el.closest(".ui-page, :jqmData(role='page')"); // expose some private props to other methods
        ...
        this._addPanelClasses(); // if animating, add the class to do so
      ...
      _getPosDisplayClasses: function _getPosDisplayClasses(prefix) {
      ...
      _handleCloseClick: function _handleCloseClick(event) {
        if (!event.isDefaultPrevented()) {
          this.close();
        }
      },
      ...
      _positionPanel: function _positionPanel(scrollToTop) {
        var self = this,
      ...
      _bindSwipeEvents: function _bindSwipeEvents() {
        var self = this,
            area = self._modal ? self.element.add(self._modal) : self.element; // on swipe, close the panel
    ...
    $.widget("mobile.table", {
      ...
      /* create */
      {
        var table = this.element,
            trs = table.find("thead tr"); // updating headers on refresh (fixes #5880)

        this._setHeaders(); // Iterate over the trs


        trs.each(function () {
          var columnCount = 0; // Iterate over the children of the tr
  ...
  (function ($, undefined) {
    $.widget("mobile.table", $.mobile.table, {
      ...
      _addToggles: function _addToggles(menu, keep) {
        var inputs,
            checkboxIndex = 0,
            opts = this.options,
            container = menu.controlgroup("container"); // allow update of menu on refresh (fixes #5880)
        ...
        this.headers.not("td").each(function () {
          ...
          if (priority) {
            ...
            cells.addClass(opts.classes.priorityPrefix + priority); // Make sure the (new?) checkbox is associated with its header via .jqmData() and
            ...
            jqmData("header", header).jqmData("cells", cells); // Associate the checkbox with the header
      ...
      _enhanceColToggle: function _enhanceColToggle() {
        ...
        menu = $("<fieldset></fieldset>").controlgroup(); // set extension here, send "false" to trigger build/rebuild
      ...
      _refresh: function _refresh(create) {
        var headers, hiddenColumns, index; // Calling _super() here updates this.headers

        this._super(create);

        if (!create && this.options.mode === "columntoggle") {
          headers = this.headers;
          hiddenColumns = []; // Find the index of the column header associated with each old checkbox among the
          ...
          this._addToggles(this._menu, create); // At this point all columns are visible, so uncheck the checkboxes that correspond to
  ...
  (function ($, undefined) {
    $.widget("mobile.table", $.mobile.table, {
      ...
      _create: function _create() {
        this._super(); // If it's not reflow mode, return here.
      ...
      _updateReflow: function _updateReflow() {
        var table = this,
            opts = this.options; // get headers in reverse order so that top-level headers are appended last
      ...
      _addLabels: function _addLabels(cells, label, contents) {
  ...
  (function ($, undefined) {
    ...
    var defaultFilterCallback = function defaultFilterCallback(index, searchValue) {
    ...
    $.widget("mobile.filterable", {
      ...
      _filterItems: function _filterItems(val) {
        var idx,
            callback,
            length,
            dst,
        ...
        if (val != null) {
          ...
          length = filterItems.length; // Partition the items into those to be hidden and those to be shown
      ...
      _setInput: function _setInput(selector) {
        var search = this._search; // Stop a pending filter operation
  ...
  (function ($, undefined) {
    ...
    var replaceSetOptions = function replaceSetOptions(self, orig) {
      return function (options) {
        ...
        origDefaultFilterCallback = $.mobile.filterable.prototype.options.filterCallback; // Override the default filter callback with one that does not hide list dividers
    ...
    $.widget("mobile.filterable", $.mobile.filterable, {
      ...
      _handleCreate: function _handleCreate(evt) {
      ...
      _setWidget: function _setWidget(widget) {
      ...
      _setOptions: function _setOptions(options) {
        var ret = this._super(options); // Need to set the filterPlaceholder after having established the search input
      ...
      _syncTextInputOptions: function _syncTextInputOptions(options) {
        var idx,
            textinputOptions = {}; // We only sync options if the filterable's textinput is of the internally
  ...
  (function ($, window) {
    $.mobile.iosorientationfixEnabled = true; // This fix addresses an iOS bug, so return early if the UA claims it's something else.
  ...
  (function ($, window, undefined) {
    var $html = $("html"),
        $window = $.mobile.window; //remove initial build class (only present on first pageshow)
    ...
    $(window.document).trigger("mobileinit"); // support conditions
    ...
    $.extend($.mobile, {
      ...
      initializePage: function initializePage() {
        ...
        var path = $.mobile.path,
            ...
            theLocation = $.mobile.path.parseLocation(),
            hashPage = hash ? document.getElementById(hash) : undefined; // if no pages are found, create one with body's inner html
        ...
        $pages.each(function () {
          var $this = $(this); // unless the data url is already set set it to the pathname
        ...
        hideRenderingClass(); // if hashchange listening is disabled, there's no hash deeplink,
    ...
    $(function () {
      ...
      $.support.inlineSVG(); // check which scrollTop value should be used by scrolling to 1 immediately at domready
      ...
      $.mobile.defaultHomeScroll = !$.support.scrollTop || $.mobile.window.scrollTop() === 1 ? 0 : 1; //dom-ready inits
