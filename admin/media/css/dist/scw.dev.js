...
var scwBaseYear = scwDateNow.getFullYear() - 50; // How many years do want to be valid and to show in the drop-down list?

var scwDropDownYears = 51; // All language-dependent changes can be made here...
...
var scwWeekStart = 1; // The week start day for the display is taken as the week start
...
var scwWeekNumberDisplay = false; // Week numbering rules are generally based on a day in the week
// that determines the first week of the year.  ISO 8601 uses
// Thursday (day four when Sunday is day zero).  You can alter
// the base day here.
// See http://www.cl.cam.ac.uk/~mgk25/iso-time.html for more information

var scwWeekNumberBaseDay = 4; // Each of the calendar's alert message types can be disabled
// independently here.
...
var scwShowInvalidDateMsg = true,
    ...
    scwShowRangeDisablingError = true; // Set the allowed input date delimiters here...
// E.g. To set the rising slash, hyphen, full-stop (aka stop or point),
//      comma and space as delimiters use
//              var scwArrDelimiters   = ['/','-','.',',',' '];

var scwArrDelimiters = ['/', '-', '.', ',', ' ']; // Set the format for the displayed 'Today' date and for the output
// date here.
//
...
var scwZindex = 1; // Personally I like the fact that entering 31-Sep-2005 displays
// 1-Oct-2005, however you may want that to be an error.  If so,
// set scwBlnStrict = true.  That will cause an error message to
// display and the selected month is displayed without a selected
// day. Thanks to Brad Allan for his feedback prompting this feature.

var scwBlnStrict = false; // If you are using ReadOnly or Disabled fields to return the date
...
var scwClearButton = true; // The calendar will position itself aligned with the bottom left
...
var scwAutoPosition = true; // If you wish to disable any displayed day, e.g. Every Monday,
...
var scwEnabledDay = [true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true]; // You can disable any specific date (e.g. 24-Jan-2006 or Today) by
// creating an element of the array scwDisabledDates as a date object
// with the value you want to disable.  Date ranges can be disabled
// by placing an array of two values (Start and End) into an element
// of this array.

var scwDisabledDates = new Array(); // e.g. To disable 10-Dec-2005:
...
var scwActiveToday = true; // Dates that are out of the displayed month are shown at the start
...
var scwTargetEle,
    ...
    scwRangeDisablingError; // Add a method to format a date into the required pattern
...
function scwShow(scwEle, scwSource) {
  ...
  if (scwSource.tagName) // Second parameter isn't an event it's an element
    ...
    {
      var scwSourceEle = scwSource.target ? scwSource.target : scwSource.srcElement; // Stop the click event that opens the calendar from bubbling up to
  ...
  scwTriggerEle = scwSourceEle; // Take any parameters that there might be from the third onwards as
  ...
  scwSeedDate = scwDateNow; // Find the date and Strip space characters from start and
  // end of date input.
  ...
  if (new Date(scwBaseYear + scwDropDownYears, 0, 0) > scwDateNow && new Date(scwBaseYear, 0, 0) < scwDateNow || scwClearButton && (scwEle.readOnly || scwEle.disabled)) {
  ...
  if (scwDateValue.length == 0) {
    ...
    var scwInputFormat = function scwInputFormat() {
      var scwArrSeed = new Array(),
          scwArrInput = scwDateValue.split(new RegExp('[\\' + scwArrDelimiters.join('\\') + ']+', 'g')); // "Escape" all the user defined date delimiters above -
      ...
      scwBlnFullInputDate = false;
      scwDateOutputFormat = scwDateOutputFormat.toUpperCase(); // List all the allowed letters in the date format

      var template = ['D', 'M', 'Y']; // Prepare the sequence of date input elements
      ...
      var scwDateSequence = result.join(''); // Separate the elements of the date input

      switch (scwArrInput.length) {
        case 1:
          {
            if (scwDateOutputFormat.indexOf('Y') > -1 && scwArrInput[0].length > scwDateOutputFormat.lastIndexOf('Y')) {
              scwArrSeed[0] = parseInt(scwArrInput[0].substring(scwDateOutputFormat.indexOf('Y'), scwDateOutputFormat.lastIndexOf('Y') + 1), 10);
            } else {
              scwArrSeed[0] = 0;
            }

            if (scwDateOutputFormat.indexOf('M') > -1 && scwArrInput[0].length > scwDateOutputFormat.lastIndexOf('M')) {
              scwArrSeed[1] = scwArrInput[0].substring(scwDateOutputFormat.indexOf('M'), scwDateOutputFormat.lastIndexOf('M') + 1);
            } else {
              scwArrSeed[1] = '6';
            }

            if (scwDateOutputFormat.indexOf('D') > -1 && scwArrInput[0].length > scwDateOutputFormat.lastIndexOf('D')) {
              scwArrSeed[2] = parseInt(scwArrInput[0].substring(scwDateOutputFormat.indexOf('D'), scwDateOutputFormat.lastIndexOf('D') + 1), 10);
            } else {
              scwArrSeed[2] = 1;
            }
      ...
      var scwExpValDay = new RegExp('^(0?[1-9]|[1-2][0-9]|3[0-1])$'),
          ...
          scwExpValYear = new RegExp('^([0-9]{1,2}|[0-9]{4})$'); // Apply validation and report failures
    ...
    scwArrSeedDate = scwInputFormat(); // So now we have the Year, Month and Day in an array.
  ...
  for (var i = 0; i < scwDisabledDates.length; i++) {
    if (!(_typeof(scwDisabledDates[i]) == 'object' && scwDisabledDates[i].constructor == Date)) {
      if (_typeof(scwDisabledDates[i]) == 'object' && scwDisabledDates[i].constructor == Array) {
        var scwPass = true;
        ...
        if (scwDisabledDates[i].length != 2) {
          ...
          for (var j = 0; j < scwDisabledDates[i].length; j++) {
            if (!(_typeof(scwDisabledDates[i][j]) == 'object' && scwDisabledDates[i][j].constructor == Date)) {
              if (scwShowRangeDisablingError) {
  ...
  scwID('scwMonths').options.selectedIndex = scwMonthSum % 12; // Check whether or not dragging is allowed and display drag handle if necessary

  scwID('scwDrag').style.display = scwAllowDrag ? '' : 'none'; // Display the month

  scwShowMonth(0); // Position the calendar box
  ...
  if (scwAutoPosition) {
    var scwWidth = parseInt(scwID('scw').offsetWidth, 10),
        ...
        scwWindowLeft = document.body && document.body.scrollLeft ? document.body.scrollLeft //DOM compliant
    ...
    offsetLeft -= offsetLeft - scwWidth + parseInt(scwTargetEle.offsetWidth, 10) >= scwWindowLeft && offsetLeft + scwWidth > scwWindowLeft + scwWindowWidth ? scwWidth - parseInt(scwTargetEle.offsetWidth, 10) : 0;
    offsetTop -= offsetTop - scwHeight - parseInt(scwTargetEle.offsetHeight, 10) >= scwWindowTop && offsetTop + scwHeight > scwWindowTop + scwWindowHeight ? scwHeight + parseInt(scwTargetEle.offsetHeight, 10) : 0;
  }
  ...
  scwID('scwIframe').style.visibility = 'inherit'; // Show it on the page
...
function scwHide() {
  ...
  if (typeof scwNextAction != 'undefined' && scwNextAction != null) {
    scwNextActionReturn = scwNextAction(); // Explicit null set to prevent closure causing memory leak
...
function scwShowMonth(scwBias) {
  ...
  var scwShowDate = new Date(Date.parse(new Date().toDateString())),
      scwStartDate = new Date(); // Set the time to the middle of the day so that the handful of
  ...
  scwShowDate.setFullYear(scwBaseYear + Math.floor(scwMonthSum / 12), scwMonthSum % 12, 1); // If the Week numbers are displayed, shift the week day names to the right.

  scwID('scwWeek_').style.display = scwWeekNumberDisplay ? '' : 'none'; // Opera has a bug with setting the selected index.
  ...
  if (scwTemp > -1 && scwTemp < 12 * scwDropDownYears) {
    var scwSetOutput = function scwSetOutput(scwOutputDate) {
    ...
    var scwCellOutput = function scwCellOutput(scwEvt) {
    ...
    var scwChangeClass = function scwChangeClass(scwEvt) {
      var scwEle = scwEventTrigger(scwEvt);
    ...
    var scwEventTrigger = function scwEventTrigger(scwEvt) {
      if (!scwEvt) {
        scwEvt = event;
      }
    ...
    var scwWeekNumber = function scwWeekNumber(scwInDate) {
      // The base day in the week of the input date
      var scwInDateWeekBase = new Date(scwInDate);
      scwInDateWeekBase.setDate(scwInDateWeekBase.getDate() - scwInDateWeekBase.getDay() + scwWeekNumberBaseDay + (scwInDate.getDay() > scwWeekNumberBaseDay ? 7 : 0)); // The first Base Day in the year
      ...
      var scwWeekNo = '0' + (Math.round((scwInDateWeekBase - scwFirstBaseDay) / 604800000, 0) + 1); // Return the last two characters in the week number string
    ...
    scwShowDate.setDate((scwShowDate.getDay() - scwWeekStart < 0 ? -6 : 1) + scwWeekStart - scwShowDate.getDay()); // This statement moved by Michael Cerveny to make version 3.55

    var scwCompareDateValue = new Date(scwShowDate.getFullYear(), scwShowDate.getMonth(), scwShowDate.getDate()).valueOf();
    scwStartDate = new Date(scwShowDate);

    if (new Date(scwBaseYear + scwDropDownYears, 0, 0) > scwDateNow && new Date(scwBaseYear, 0, 0) < scwDateNow) {
      var scwNowOutput = function scwNowOutput() {
      ...
      if (scwDisabledDates.length == 0) {
        ...
        for (var k = 0; k < scwDisabledDates.length; k++) {
          if (!scwActiveToday || !scwParmActiveToday || _typeof(scwDisabledDates[k]) == 'object' && (scwDisabledDates[k].constructor == Date && scwDateNow.valueOf() == scwDisabledDates[k].valueOf() || scwDisabledDates[k].constructor == Array && scwDateNow.valueOf() >= scwDisabledDates[k][0].valueOf() && scwDateNow.valueOf() <= scwDisabledDates[k][1].valueOf())) {
            scwNow.onclick = null;
            scwNow.className = 'scwNowDisabled';
      ...
      if (scwRows.nodeType == 1 && scwRows.tagName == 'TR') {
        ...
        for (j = 1; j < scwRows.childNodes.length; j++) {
          ...
          if (scwCols.nodeType == 1 && scwCols.tagName == 'TD') {
            ...
            for (var k = 0; k < scwDisabledDates.length; k++) {
              if (_typeof(scwDisabledDates[k]) == 'object' && scwDisabledDates[k].constructor == Date && scwCompareDateValue == scwDisabledDates[k].valueOf()) {
                scwDisabled = true;
              } else {
                if (_typeof(scwDisabledDates[k]) == 'object' && scwDisabledDates[k].constructor == Array && scwCompareDateValue >= scwDisabledDates[k][0].valueOf() && scwCompareDateValue <= scwDisabledDates[k][1].valueOf()) {
                  scwDisabled = true;
                }
              }
            }
...
document.write("</tbody>" + "<tfoot>" + "<tr id='scwFoot'>" + "<td colspan='8' style='padding:0px;'>" + "<table width='100%'>" + "<tr>" + "<td id='scwClear' class='scwClear'>" + "<input type='button' id='scwClearButton' class='scwClear' " + "onclick='scwTargetEle.value = \"\";scwHide();' />" + "</td>" + "<td class='scwNow' id='scwNow'></td>" + "</tr>" + "</table>" + "</td>" + "</tr>" + "</tfoot>" + "</table>" + "</td>" + "</tr>" + "</table>");
