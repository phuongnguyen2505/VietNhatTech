...
String.prototype.parseColor = function () {
  ...
  if (this.slice(0, 4) == 'rgb(') {
    ...
    if (this.slice(0, 1) == '#') {
      if (this.length == 4) for (var i = 1; i < 4; i++) {
...
var Effect = {
  ...
  tagifyText: function tagifyText(element) {
  ...
  multiple: function multiple(element, effect) {
    var elements;
    if ((_typeof(element) == 'object' || Object.isFunction(element)) && element.length) elements = element;else elements = $(element).childNodes;
  ...
  toggle: function toggle(element, effect) {
    element = $(element);
...
Effect.Parallel = Class.create(Effect.Base, {
  initialize: function initialize(effects) {
  ...
  finish: function finish(position) {
...
Effect.Tween = Class.create(Effect.Base, {
  initialize: function initialize(object, from, to) {
...
Effect.Opacity = Class.create(Effect.Base, {
  initialize: function initialize(element) {
    ...
    if (!this.element) throw Effect._elementDoesNotExistError; // make this work on IE on elements without 'layout'
...
Effect.Scale = Class.create(Effect.Base, {
  initialize: function initialize(element, percent) {
    this.element = $(element);
    if (!this.element) throw Effect._elementDoesNotExistError;
    var options = Object.extend({
      scaleX: true,
...
Effect.Highlight = Class.create(Effect.Base, {
  ...
  setup: function setup() {
    ...
    if (!this.options.restorecolor) this.options.restorecolor = this.element.getStyle('background-color'); // init color calculations
...
Effect.Appear = function (element) {
  ...
  var options = Object.extend({
    ...
    beforeSetup: function beforeSetup(effect) {
...
Effect.Puff = function (element) {
  ...
  return new Effect.Parallel([new Effect.Scale(element, 200, {
    ...
    duration: 1.0,
    beforeSetupInternal: function beforeSetupInternal(effect) {
...
Effect.BlindDown = function (element) {
  ...
  return new Effect.Scale(element, 100, Object.extend({
    ...
    restoreAfterFinish: true,
    afterSetup: function afterSetup(effect) {
...
Effect.SlideDown = function (element) {
  ...
  return new Effect.Scale(element, 100, Object.extend({
    ...
    afterUpdateInternal: function afterUpdateInternal(effect) {
...
Effect.SlideUp = function (element) {
  ...
  var oldInnerBottom = element.down().getStyle('bottom');
  var elementDimensions = element.getDimensions();
  return new Effect.Scale(element, window.opera ? 0 : 1, Object.extend((_Object$extend = {
    scaleContent: false,
    scaleX: false,
    scaleMode: 'box',
    scaleFrom: 100
  }, _defineProperty(_Object$extend, "scaleMode", {
    originalHeight: elementDimensions.height,
    originalWidth: elementDimensions.width
  }), _defineProperty(_Object$extend, "restoreAfterFinish", true), _defineProperty(_Object$extend, "afterSetup", function afterSetup(effect) {
    effect.element.makePositioned();
    effect.element.down().makePositioned();
    if (window.opera) effect.element.setStyle({
      top: ''
  ...
  }), _defineProperty(_Object$extend, "afterUpdateInternal", function afterUpdateInternal(effect) {
    effect.element.down().setStyle({
      bottom: effect.dims[0] - effect.element.clientHeight + 'px'
    });
  }), _defineProperty(_Object$extend, "afterFinishInternal", function afterFinishInternal(effect) {
    effect.element.hide().undoClipping().undoPositioned();
    effect.element.down().undoPositioned().setStyle({
      bottom: oldInnerBottom
    });
  }), _Object$extend), arguments[1] || {}));
...
Effect.Shrink = function (element) {
  ...
  return new Effect.Parallel([new Effect.Opacity(element, {
    ...
    beforeStartInternal: function beforeStartInternal(effect) {
...
Effect.Pulsate = function (element) {
  ...
  var reverser = function reverser(pos) {
...
Effect.Transform = Class.create({
  initialize: function initialize(tracks) {
    this.tracks = [];
...
;
Effect.Methods = {
  morph: function morph(element, style) {
    element = $(element);
  ...
  visualEffect: function visualEffect(element, effect, options) {
    element = $(element);
  ...
  highlight: function highlight(element, options) {
    element = $(element);
