...
function docheck(status, from_) {
  ...
  if (from_ > 0) document.frm.chkall.checked = status;
}
...
function docheckone() {
  ...
  if (alen > 0) {
    for (var i = 0; i < alen; i++) {
      if (document.frm.chkid[i].checked == false) isChecked = false;
    }
  } else {
    if (document.frm.chkid.checked == false) isChecked = false;
  }
...
function calculate() {
  ...
  if (alen > 0) {
    for (var i = 0; i < alen; i++) {
      if (document.frm.chkid[i].checked == true) strchon += "," + document.frm.chkid[i].value;
    }
  } else {
    if (document.frm.chkid.checked == true) strchon = document.frm.chkid.value;
  }
...
function showtime(id, lang) {
  var navName = navigator.appName; //alert("navName = "+navName);
  ...
  if (typeof lang != 'undefined' && lang == 'vi') {
    ...
    if (now.getMonth() > 9) timeValue += ' - ' + now.getMonth();else timeValue += ' - 0' + now.getMonth();
    ...
    if (now.getMonth() > 9) timeValue += ' &nbsp;' + now.getMonth();else timeValue += ' &nbsp;0' + now.getMonth();
    timeValue += ' - ' + now.getDate();
    if (navName == 'Netscape') timeValue += ' - ' + (now.getYear() + 1900);else timeValue += ' - ' + now.getYear();
  }
