...
function cat_chu_cuoi($str, $sotu)
{
	$str = trim($str);
	$l = strlen($str);
	$dem = 0;
...
function bo_chu_cuoi($str, $sotu)
{
	$str = trim($str);
	$l = strlen($str);

	$chuoi = substr($str, 0, ($l - strlen(cat_chu_cuoi($str, $sotu))));

	return $chuoi;
}

function get_day($lang)
{
	...
	switch ($weekday) {
		case 'monday':
			$weekday = ($lang == 'vi') ? 'Thứ hai' : 'Monday';
			break;
		case 'tuesday':
			$weekday = ($lang == 'vi') ? 'Thứ ba' : 'tuesday';
			break;
		case 'wednesday':
			$weekday = ($lang == 'vi') ? 'Thứ tư' : 'Wednesday';
			break;
		case 'thursday':
			$weekday = ($lang == 'vi') ? 'Thứ năm' : 'Thursday';
			break;
		case 'friday':
			$weekday = ($lang == 'vi') ? 'Thứ sáu' : 'Friday';
			break;
		case 'saturday':
			$weekday = ($lang == 'vi') ? 'Thứ bảy' : 'Saturday';
			break;
		default:
			$weekday = ($lang == 'vi') ? 'Chủ nhật' : 'Sunday';
			break;
	}
	if ($lang == 'vi')
		$t = $weekday . ', ngày ' . date('d') . ' Tháng ' . date('m') . ' Năm ' . date('Y');
	else
		$t = $weekday . ', ' . date('F') . ' ' . date('jS') . ', ' . date('Y');
	return $t;
}
...
function magic_quote2($str, $id_connect = false)
{
	if (is_array($str)) {
...
function paging($r, $url = '', $curPg = 1, $mxR = 5, $mxP = 5, $class_paging = '')
{
	...
	if ($totalRows > $mxR) {
		...
		$paging .= " <a href='" . $url . "&curPage=" . ($curPg - 1) . "' class=\"{$class_paging}\" >Trang trước</a> "; //ve truoc
		#}
		$paging .= $paging1;
		#if(((int)(($curPg-1)/$mxP+1)*$mxP) < $totalPages)  
		#{
		#$paging .=" <a href='".$url."&curPage=".($end+1)."' class=\"{$class_paging}\" >&#8250;</a> "; //ke
		$paging .= " <a href='" . $url . "&curPage=" . ($curPg + 1) . "' class=\"{$class_paging}\" >Trang sau</a> "; //ke

		$paging .= " <a href='" . $url . "&curPage=" . ($totalPages) . "' class=\"{$class_paging}\" >Trang cuối</a> "; //ve cuoi
		#}
	}
	$r3['curPage'] = $curPg;
	$r3['source'] = $r2;
...
function paging_home($r, $url = '', $curPg = 1, $mxR = 5, $mxP = 5, $class_paging = '')
{
	...
	if ($totalRows > $mxR) {
		...
		for ($i = 1; $i <= $totalPages; $i++) {
			if (($i > ((int) (($curPg - 1) / $mxP)) * $mxP) && ($i <= ((int) (($curPg - 1) / $mxP + 1)) * $mxP)) {
				if ($start == 1)
					$start = $i;
				if ($i == $curPg) {
					$paging1 .= " <a class='active_p'>" . $i . "</a> "; //dang xem
				} else {
		...
		#if($curPg>$mxP)
		#{
		$paging .= " <a href='" . $url . "' class=\"{$class_paging} pag_li\" >First</a> "; //ve dau

		#$paging .=" <a href='".$url."&curPage=".($start-1)."' class=\"{$class_paging}\" >&#8249;</a> "; //ve truoc
		$paging .= " <a href='" . $url . "&p=" . ($curPg - 1) . "' class=\"{$class_paging} pag_li\" >Prev</a> "; //ve truoc
		#}
		$paging .= $paging1;
		#if(((int)(($curPg-1)/$mxP+1)*$mxP) < $totalPages)  
		#{
		#$paging .=" <a href='".$url."&curPage=".($end+1)."' class=\"{$class_paging}\" >&#8250;</a> "; //ke
		$paging .= " <a href='" . $url . "&p=" . ($curPg + 1) . "' class=\"{$class_paging} pag_li\" >Next</a> "; //ke

		$paging .= " <a href='" . $url . "&p=" . ($totalPages) . "' class=\"{$class_paging} pag_li\" >Last</a> "; //ve cuoi
		#}
	}
	$r3['curPage'] = $curPg;
	$r3['source'] = $r2;
