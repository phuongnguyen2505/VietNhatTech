...
<form name="frm" method="post" action="index.php?com=mxh&act=save" enctype="multipart/form-data" class="nhaplieu">
    ...
    <?php }?>
    <b>Hình ảnh:</b> <input type="file" name="file" /> <strong>.png &nbsp;&nbsp; (width:45px -
        height:46px)</strong><br />

    <b>Số thứ tự</b> <input type="text" name="stt" value="<?=isset($item['stt'])?$item['stt']:1?>"
        style="width:30px"><br>

    <b>Hiển thị</b> <input type="checkbox" name="hienthi"
        <?=(!isset($item['hienthi']) || $item['hienthi']==1)?'checked="checked"':''?>><br />
