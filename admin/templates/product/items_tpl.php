...
function select_onchange2() {
    ...
    window.location = "index.php?com=product&act=man&id_item=" + c.value;
    return true;
}
</script>
<?php
...
function get_main_item()
	{
		$sql="select ten_vi,id from table_product_item where hienthi =1 and id_cat=".$_REQUEST['id_cat']." order by stt asc";
		$stmt=mysql_query($sql);
		$str='
...
<table class="blue_table">
    ...
    <tr>
        ...
        <td style="width:20%;"><a
                href="index.php?com=product&act=edit&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id_item=<?=$items[$i]['id_item']?>&id=<?=$items[$i]['id']?>"
        ...
        <a href="index.php?com=product&act=man&sptb=<?=$items[$i]['id']?><?php if($_REQUEST['curPage']!='') echo'&curPage='. $_REQUEST['curPage'];?>"><img src="media/images/yes-km.gif" border="0" /></a>
		<? 
		}
		else
		{
		?>
         <a href="index.php?com=product&act=man&sptb=<?=$items[$i]['id']?><?php if($_REQUEST['curPage']!='') echo'&curPage='. $_REQUEST['curPage'];?>"><img src="media/images/no-km.gif"  border="0"/></a>
         <?php
		 }?>      
         </td>      -->
        <td style="width:5%;">
		...
		?>
            ...
            <a
                href="index.php?com=product&act=man&hienthi=<?=$items[$i]['id']?><?php if($_REQUEST['curPage']!='') echo'&curPage='. $_REQUEST['curPage'];?>"><img
        ...
        <td style="width:5%;"><a
                href="index.php?com=product&act=edit&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id_item=<?=$items[$i]['id_item']?>&id=<?=$items[$i]['id']?>"><img
                    src="media/images/edit.png" /></a></td>
        <td style="width:5%;"><a href="index.php?com=product&act=delete&id=<?=$items[$i]['id']?>"
                onClick="if(!confirm('Xác nhận xóa')) return false;"><img src="media/images/delete.png" /></a></td>
    </tr>
    <?php	}?>
</table>
<a href="index.php?com=product&act=add"><img src="media/images/add.jpg"
