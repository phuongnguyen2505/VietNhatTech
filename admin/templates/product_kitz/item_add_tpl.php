...
function select_onchange()
	{				
		...
		window.location ="index.php?com=product_kitz&act=<?php if($_REQUEST['act']=='edit') echo 'edit'; else echo 'add';?><?php if($_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&id_list="+a.value;	
		return true;
	}
	...
	{				
		...
		window.location ="index.php?com=product_kitz&act=<?php if($_REQUEST['act']=='edit') echo 'edit'; else echo 'add';?><?php if($_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&id_list="+a.value+"&id_cat="+b.value;	
		return true;
	}
</script>
<?php
function get_main_list()
	{
		$sql="select * from table_product_kitz_list order by stt";
...
function get_main_cat()
	...
	{
		$sql="select ten_vi,id from table_product_kitz_item where hienthi=1 and id_list=".$_REQUEST['id_list']." and id_cat=".$_REQUEST['id_cat']." order by stt asc";
		$stmt=mysql_query($sql);
		$str='
...
?>
<form name="frm" method="post" action="index.php?com=product_kitz&act=save&curPage=<?=$_REQUEST['curPage']?>" enctype="multipart/form-data" class="nhaplieu">	 
	...
	<?php }?>
	<b>Hình ảnh:</b> <input type="file" name="file" /> <?=_product_type?> - <strong>Tỉ lệ chuẩn: width 630px - height 690px</strong><br />
    <br />
    ...
    <div id="info_deals" class="usual"> 
		...
		<div id="tab1" class="content_tab">
			...
			<br><br />
			<b>Mô tả(VN):</b> <textarea name="mota_vi" id="mota_vi" cols="45" rows="5"><?=@$item['mota_vi']?></textarea><br /><br>
			<b>Nội dung(VI)</b><br/>
			<div>
			<textarea name="noidung_vi" id="noidung_vi"><?=str_replace('yoshitake.net.vn', 'vietnhatvalves.com',stripcslashes($item['noidung_vi']))?></textarea></div>
			<br/> 	
  ...
  <br><br />	
  <b>Mô tả(EN):</b> <textarea name="mota_en" id="mota_en" cols="45" rows="5"><?=@$item['mota_en']?></textarea><br /><br>
  <b>Nội dung(EN)</b><br/>
	<div>
  ...
  <br><br />
   <b>Nội dung(JP)</b><br/>
	...
	<input type="button" value="Thoát" onclick="javascript:window.location='index.php?com=product_kitz&act=man'" class="btn" />
</form>
