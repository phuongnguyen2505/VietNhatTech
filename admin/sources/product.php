...
function get_items()
{
    ...
    if (isset($_REQUEST['sptb']) && $_REQUEST['sptb'] != '') {
        $id_up = $_REQUEST['sptb'];
        $sql_sp = "SELECT id,sptb FROM table_product where id='".$id_up."' ";
        $d->query($sql_sp);
        $cats= $d->result_array();
        $time=time();
        $hienthi=$cats[0]['sptb'];
        if($hienthi==0) {
            $sqlUPDATE_ORDER = "UPDATE table_product SET sptb ='$time' WHERE  id = ".$id_up."";
            $resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
        }
        else
        {
            $sqlUPDATE_ORDER = "UPDATE table_product SET sptb =0  WHERE  id = ".$id_up."";
            $resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
        }    
    }
    // ----------------------------------------------------------------------------------------
    ...
    if (isset($_GET['capnhat']) && $_GET['capnhat'] == '1') {
        foreach ($items as $key => $value) {
            $data['ten_en']=$value['ten_vi'];
            $data['mota_en']=$value['mota_vi'];
            $data['noidung_en']=$value['noidung_vi'];
            $data['title_en']=$value['title_vi'];
            $data['keywords_en']=$value['keywords_vi'];
            $data['description_en']=$value['description_vi'];
            $d->reset();
            $d->setTable('product');
...
function get_lists()
{
    ...
    if (isset($_REQUEST['noibat']) && $_REQUEST['noibat'] !== '') {
        $id_up = $_REQUEST['noibat'];
        $sql_sp = "select noibat from table_product_list where id='".$id_up."'";
        $d->query($sql_sp);
        $cats= $d->result_array();
        $time=time();
        $hienthi=$cats[0]['noibat'];
...
function delete_binhluan()
{
    ...
    }else { transfer("Không nhận được dữ liệu", "index.php?com=product&act=duyetbl");
    }
}
...
function save_tags()
{
    ...
    if($id) {
        ...
        $d->setWhere('id', $id);
        if($d->update($data)) {
            redirect("index.php?com=product&act=man_tags&idp=$idp");
        } else {
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_tags&idp=$idp");
        }
    }else{    
        $idp=$_POST['idp'];
        for($i=0; $i<5;$i++)
...
function delete_tag()
{
    ...
    if(isset($_GET['id'])) {
        ...
        if($d->query($sql)) {
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_tags&idp=$idp");
        }
    }
}
