...
case "man":
    get_items();
    $template = "congnghe/items";
    break;
case "add":
    $template = "congnghe/item_add";
    break;
case "edit":        
    get_item();        
    $template = "congnghe/item_add";
...
case "man_cat":
    get_cats();
    $template = "congnghe/cats";
    break;
case "add_cat":
    $template = "congnghe/cat_add";
    break;
case "edit_cat":
    get_cat();
    $template = "congnghe/cat_add";
...
function get_items()
{
    ...
    $sql = "select * from #_news where loaitin='cong-nghe' ";
    ...
    $url="index.php?com=congnghe&act=man";
    $maxR=20;
    $maxP=4;
...
function get_item()
{
    ...
    if(!$id) {
        transfer("Không nhận được dữ liệu", "index.php?com=congnghe&act=man");
    }
    
    $sql = "select * from #_news where id='".$id."'";
    $d->query($sql);
    if($d->num_rows()==0) { transfer("Dữ liệu không có thực", "index.php?com=congnghe&act=man");
    }
    $item = $d->fetch_array();
}
...
function save_item()
{
    ...
    if(empty($_POST)) { transfer("Không nhận được dữ liệu", "index.php?com=congnghe&act=man");
    }
    ...
    if($id) {
        ...
        $d->setWhere('id', $id);
        if($d->update($data)) {
            redirect("index.php?com=congnghe&act=man");
        } else {
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=congnghe&act=man");
        }
    }else{
        ...
        if($d->insert($data)) {
            ...
            transfer("Lưu dữ liệu bị lỗi", "index.php?com=congnghe&act=man");
        }
    }
}
...
function delete_item()
{
    ...
    if(isset($_GET['id'])) {
        ...
        if($d->query($sql)) {
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=congnghe&act=man");
        }
        ...
        redirect("index.php?com=congnghe&act=man".$id_catt."");
    }
}
//===========================================================
function get_cats()
{
    ...
    $sql = "select * from #_news_cat where com='cong-nghe'  order by stt";
    $d->query($sql);
    $items = $d->result_array();
    
    $curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
    $url="index.php?com=congnghe&act=man_cat";
    $maxR=20;
    $maxP=4;
    $paging=paging($items, $url, $curPage, $maxR, $maxP);
    $items=$paging['source'];
...
function get_cat()
{
    ...
    $id = isset($_GET['id']) ? themdau($_GET['id']) : "";
    if(!$id) {
        transfer("Không nhận được dữ liệu", "index.php?com=congnghe&act=man_cat");
    }
    
    $sql = "select * from #_news_cat where id='".$id."'";
    $d->query($sql);
    if($d->num_rows()==0) { transfer("Dữ liệu không có thực", "index.php?com=congnghe&act=man_cat");
    }
    $item = $d->fetch_array();
}
...
function save_cat()
{
    ...
    if(empty($_POST)) { transfer("Không nhận được dữ liệu", "index.php?com=congnghe&act=man_cat");
    }
    $id = isset($_POST['id']) ? themdau($_POST['id']) : "";
    if($id) {
        ...
        $d->setWhere('id', $id);
        if($d->update($data)) {
            redirect("index.php?com=congnghe&act=man_cat&curPage=".$_REQUEST['curPage']."");
        } else {
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=congnghe&act=man_cat");
        }
    }else{
        ...
        $d->setTable('news_cat');
        if($d->insert($data)) {
            redirect("index.php?com=congnghe&act=man_cat");
        } else {
            transfer("Lưu dữ liệu bị lỗi", "index.php?com=congnghe&act=man_cat");
        }
    }
}
...
function delete_cat()
{
    ...
    if(isset($_GET['id'])) {
        ...
        if($d->query($sql)) {
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=congnghe&act=man_cat");
        }
    }else { transfer("Không nhận được dữ liệu", "index.php?com=congnghe&act=man_cat");
    }
}
?>
