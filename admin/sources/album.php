...
function get_items(){
	...
	$sql = "select * from #_news where loaitin='album' ";
...
function save_item(){
	...
	if($id){
		...
		if($photo = upload_image("file", 'gif|png|jpg|GIF|PNG|JPG',_upload_tintuc,changeTitle(substr($_FILES['file']['name'], 0,strlen($_FILES['file']['name'])-4)))){
			$data['photo'] = $photo;
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		redirect("index.php?com=album&act=man".$id_catt."");
	}
}
//===========================================================
function get_cats(){
	...
	$sql = "select * from #_news_cat where com='album'  order by stt";
	$d->query($sql);
	$items = $d->result_array();
