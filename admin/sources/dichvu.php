...
function get_item()
{
    ...
    $d->query($sql);
    if ($d->num_rows() == 0) {
        transfer("Dữ liệu không có thực", "index.php?com=dichvu&act=man");
    }
    $item = $d->fetch_array();
}
...
function save_item()
{
    ...
    if ($id) {
        ...
        if ($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG|PNG', _upload_dichvu, $file_name)) {
            $data['photo'] = $photo;
            $data['thumb'] = create_thumb($data['photo'], 1366, 768, _upload_dichvu, $file_name, 1);
            $d->setTable('dichvu');
            $d->setWhere('id', $id);
            $d->select();
            if ($d->num_rows() > 0) {
...
function get_cat()
{
    ...
    $d->query($sql);
    if ($d->num_rows() == 0) {
        transfer("Dữ liệu không có thực", "index.php?com=dichvu&act=man_cat");
    }
    $item = $d->fetch_array();
}
...
function get_loai()
{
    ...
    $d->query($sql);
    if ($d->num_rows() == 0) {
        transfer("Dữ liệu không có thực", "index.php?com=dichvu&act=man_item");
    }
    $item = $d->fetch_array();
}
...
function get_list()
{
    ...
    $d->query($sql);
    if ($d->num_rows() == 0) {
        transfer("Dữ liệu không có thực", "index.php?com=dichvu&act=man_list");
    }
    $item = $d->fetch_array();
}
