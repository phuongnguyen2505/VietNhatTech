...
function get_item()
{
    ...
    $d->query($sql);
    if ($d->num_rows() == 0) {
        transfer("Dữ liệu không có thực", "index.php?com=news&act=man");
    }
    $item = $d->fetch_array();
}
...
function save_item()
{
    ...
    if ($id) {
        ...
        if ($photo = upload_image("file", 'jpg|png|gif|JPG|PNG|GIF', _upload_tintuc, $file_name)) {
            $data['photo'] = $photo;
            $data['thumb'] = create_thumb($data['photo'], 854, 480, _upload_tintuc, $file_name, 1);
            $d->setTable('news');
            $d->setWhere('id', $id);
            $d->select();
            if ($d->num_rows() > 0) {
...
function get_cat()
{
    ...
    $d->query($sql);
    if ($d->num_rows() == 0) {
        transfer("Dữ liệu không có thực", "index.php?com=news&act=man_cat");
    }
    $item = $d->fetch_array();
}
