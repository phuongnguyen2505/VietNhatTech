...
function get_items(){
	...
	$sql = "select * from #_thanhvien order by username";
	$d->query($sql);
...
function edit(){
	...
	if(!empty($_POST)){
		...
		$sql = "select * from #_nguoidung where username='".$_SESSION['login']['username']."'";
		$d->query($sql);
		if($d->num_rows() == 1){
			$row = $d->fetch_array();
			if($row['password'] != md5($_POST['oldpassword'])) transfer("Mật khẩu không chính xác","index.php?com=user&act=edit");
	...
	$sql = "select * from #_thanhvien where username='".$_SESSION['login']['username']."'";
	$d->query($sql);
	if($d->num_rows() == 1){
		$item = $d->fetch_array();
	}
