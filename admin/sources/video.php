...
function get_items(){
	...
	if($spdc1==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_video SET noibat ='".$noibat."' WHERE  id = ".$id_up."";
...
function save_item(){
	...
	if($id){
		...
		if($photo = upload_image("file", 'jpg|png|gif', _upload_hinhanh,$file_name)){
			...
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_hinhanh.$row['image']);	
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		// xoa hinh anh
		$d->reset();
		$sql = "select id, image from #_video where id='".$id."'";
		$d->query($sql);
		if($d->num_rows()>0){
			while($row = $d->fetch_array()){
				delete_file(_upload_video.$row['image']);
			}
