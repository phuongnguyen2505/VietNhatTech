...
(function () {
  ...
  tinymce.create("tinymce.plugins.Lists", {
    init: function init(y) {
      ...
      function E(U) {
        return U.keyCode === tinymce.VK.TAB && !(U.altKey || U.ctrlKey) && (y.queryCommandState("InsertUnorderedList") || y.queryCommandState("InsertOrderedList"));
      }

      function w() {
        var U = B();
        var W = U.parentNode.parentNode;
        var V = U.parentNode.lastChild === U;
        return V && !t(W) && P(U);
      }

      function t(U) {
        if (n(U)) {
          return U.parentNode && U.parentNode.tagName === "LI";
        } else {
          return U.tagName === "LI";
        }
      }
      ...
      function F() {
        return y.selection.isCollapsed() && P(B());
      }

      function B() {
        var U = y.selection.getStart();
        return (U.tagName == "BR" || U.tagName == "") && U.parentNode.tagName == "LI" ? U.parentNode : U;
      }
      ...
      function P(U) {
        ...
        if (U.tagName === "LI") {
          return V == 0 ? true : V == 1 && (U.firstChild.tagName == "" || U.firstChild.tagName == "BR" || H(U));
        }
      ...
      function H(U) {
        var V = tinymce.grep(U.parentNode.childNodes, function (Y) {
          return Y.tagName == "LI";
        });
        var W = U == V[V.length - 1];
        var X = U.firstChild;
        return tinymce.isIE9 && W && (X.nodeValue == String.fromCharCode(160) || X.nodeValue == String.fromCharCode(32));
      }

      function T(U) {
        return U.keyCode === tinymce.VK.ENTER;
      }

      function A(U) {
        return T(U) && !U.shiftKey;
      }
      ...
      function D(U, V) {
        if (x == v || x == s || tinymce.isGecko && x == J) {
          r.cancel(V);
        }
      }

      function C() {
        var U = y.selection.getRng(true);
        var V = U.startContainer;

        if (V.nodeType == 3) {
          var W = V.nodeValue;

          if (tinymce.isIE9 && W.length > 1 && W.charCodeAt(W.length - 1) == 32) {
            return U.endOffset == W.length - 1;
          } else {
            return U.endOffset == W.length;
          }
        } else {
          if (V.nodeType == 1) {
            return U.endOffset == V.childNodes.length;
          }
        }
      ...
      function I() {
        var W = y.selection.getNode();
        var V = "h1,h2,h3,h4,h5,h6,p,div";
        var U = y.dom.is(W, V) && W.parentNode.tagName === "LI" && W.parentNode.lastChild === W;
        return y.selection.isCollapsed() && U && C();
      }

      function K(W, Y) {
        if (A(Y) && I()) {
          var X = W.selection.getNode();
          var V = W.dom.create("li");
          var U = W.dom.getParent(X, "li");
          W.dom.insertAfter(V, U);

          if (tinymce.isIE6 || tinymce.isIE7 || tinyMCE.isIE8) {
            W.selection.setCursorLocation(V, 1);
          } else {
            W.selection.setCursorLocation(V, 0);
          }
      ...
      function u(X, Z) {
        ...
        var V = X.selection.getStart();

        if (Z.keyCode != tinymce.VK.BACKSPACE || V.tagName !== "IMG") {
          return;
        }

        function W(ag) {
          var ah = ag.firstChild;
          var af = null;
        ...
        function ae(ag, af) {
          while (ag.childNodes.length > 0) {
            af.appendChild(ag.childNodes[0]);
          }
        }

        ac = V.parentNode.previousSibling;
        ...
        if (ac.tagName === "UL" || ac.tagName === "OL") {
          aa = ac;
        } else {
          if (ac.previousSibling && (ac.previousSibling.tagName === "UL" || ac.previousSibling.tagName === "OL")) {
        ...
        var ad = W(aa);
        var U = X.dom.createRng();
        U.setStart(ad, 1);
        U.setEnd(ad, 1);
        X.selection.setRng(U);
        X.selection.collapse(true);
        var Y = X.selection.getBookmark();
        var ab = V.parentNode.cloneNode(true);

        if (ab.tagName === "P" || ab.tagName === "DIV") {
          ae(ab, ad);
        } else {
          ad.appendChild(ab);
        }
        ...
        X.selection.moveToBookmark(Y);
      }

      function G(U) {
        var V = y.dom.getParent(U, "ol,ul");

        if (V != null) {
          var W = V.lastChild;
          y.selection.setCursorLocation(W, 0);
        }
      }
      ...
      y.onInit.add(function () {
        y.editorCommands.addCommands({
          outdent: function outdent() {
            var V = y.selection,
                W = y.dom;

            function U(X) {
              X = W.getParent(X, W.isBlock);
              return X && parseInt(y.dom.getStyle(X, "margin-left") || 0, 10) + parseInt(y.dom.getStyle(X, "padding-left") || 0, 10) > 0;
            }

            return U(V.getStart()) || U(V.getEnd()) || y.queryCommandState("InsertOrderedList") || y.queryCommandState("InsertUnorderedList");
          }
        }, "state");
      });
      y.onKeyUp.add(function (V, W) {
        if (x == v) {
          V.execCommand(W.shiftKey ? "Outdent" : "Indent", true, null);
          x = N;
          return r.cancel(W);
        } else {
          if (x == s) {
            var U = B();
            var Y = V.settings.list_outdent_on_enter === true || W.shiftKey;
            V.execCommand(Y ? "Outdent" : "Indent", true, null);
            ...
            if (x == J) {
              if (tinymce.isIE6 || tinymce.isIE7 || tinymce.isIE8) {
                var X = V.getDoc().createTextNode("\uFEFF");
                V.selection.getNode().appendChild(X);
              } else {
                if (tinymce.isIE9 || tinymce.isGecko) {
                  V.execCommand("Outdent");
                  return r.cancel(W);
                }
              }
            }
      ...
      function R(V, X) {
        if (T(X)) {
          ...
          if (U) {
            var W = U.parentNode;
            var Y = W && W.parentNode;

            if (Y && Y.nodeName == "LI" && Y.firstChild == W && U == W.firstChild) {
              L(Y, W);
            }
          }
        }
      ...
      function S(V, X) {
        if (T(X)) {
          ...
          if (V.dom.select("ul li", U).length === 1) {
            var W = U.firstChild;
            L(U, W);
          }
        }
      ...
      function Q(V, Z) {
        function W(ad, aa) {
          var ac = [];
          var ae = new tinymce.dom.TreeWalker(aa, ad);

          for (var ab = ae.current(); ab; ab = ae.next()) {
            if (V.dom.is(ab, "ol,ul,li")) {
              ac.push(ab);
            }
          }
        ...
        if (Z.keyCode == tinymce.VK.BACKSPACE) {
          var U = B();

          if (U) {
            var Y = V.dom.getParent(U, "ol,ul");

            if (Y && Y.firstChild === U) {
              var X = W(Y, U);
              V.execCommand("Outdent", false, X);
              V.undoManager.add();
              return r.cancel(Z);
            }
          }
      ...
      function O(V, X) {
        ...
        if (X.keyCode === tinymce.VK.BACKSPACE && V.dom.is(U, "li") && U.parentNode.firstChild !== U) {
          if (V.dom.select("ul,ol", U).length === 1) {
            var Z = U.previousSibling;
            V.dom.remove(V.dom.select("br", U));
            V.dom.remove(U, true);
            var W = tinymce.grep(Z.childNodes, function (aa) {
              return aa.nodeType === 3;
            });

            if (W.length === 1) {
              var Y = W[0];
              V.selection.setCursorLocation(Y, Y.length);
            }
    ...
    applyList: function applyList(y, v) {
      var C = this,
          z = C.ed,
          I = z.dom,
          s = [],
          ...
          w = false,
          B,
          G = z.selection.getSelectedBlocks();
      ...
      function K(Q, T) {
        var N,
            ...
            O = 3,
            L = 1,
            t = "br,ul,ol,p,div,h1,h2,h3,h4,h5,h6,table,blockquote,address,pre,form,center,dl";
    ...
    indent: function indent() {
      ...
      function t(B) {
        var y = s(B),
            D = w.getParent(B, "ol,ul"),
            C = D.tagName,
            E = w.getStyle(D, "list-style-type"),
            A = {},
            z;
    ...
    process: function process(y) {
      ...
      function x(s) {
        ...
        if (!s || s.nodeType !== 1 || !y.processEvenIfEmpty && E.length > 1 && B(s)) {
          return;
        }
    ...
    splitSafeEach: function splitSafeEach(t, s) {
    ...
    classBasedEach: function classBasedEach(v, u) {
      var w = this.ed.dom,
          s,
          t;
      e(v, function (x) {
    ...
    adjustPaddingFunction: function adjustPaddingFunction(u) {
      var s,
          v,
          t = this.ed;
    ...
    selectedBlocks: function selectedBlocks() {
      var s = this.ed;
