...
(function () {
  var each = tinymce.each,
      ...
      bookmark; // Skips text nodes that only contain whitespace since they aren't semantically important.
  ...
  tinymce.create('tinymce.plugins.Lists', {
    init: function init(ed) {
      ...
      function isTabInList(e) {
        ...
        return e.keyCode === tinymce.VK.TAB && !(e.altKey || e.ctrlKey) && (ed.queryCommandState('InsertUnorderedList') || ed.queryCommandState('InsertOrderedList'));
      }
      ...
      function getLi() {
        var n = ed.selection.getStart(); // Get start will return BR if the LI only contains a BR or an empty element as we use these to fix caret position
      ...
      function createNewLi(ed, e) {
        if (isEnterWithoutShift(e) && isEndOfListItem()) {
          ...
          ed.dom.insertAfter(li, parentLi); // Move caret to new list element.
      ...
      function imageJoiningListItem(ed, e) {
        ...
        var ul;
        if (prevSibling.tagName === 'UL' || prevSibling.tagName === 'OL') ul = prevSibling;else if (prevSibling.previousSibling && (prevSibling.previousSibling.tagName === 'UL' || prevSibling.previousSibling.tagName === 'OL')) ul = prevSibling.previousSibling;else return;
        var li = lastLI(ul); // move the caret to the end of the list item

        var rng = ed.dom.createRng();
        rng.setStart(li, 1);
        rng.setEnd(li, 1);
        ed.selection.setRng(rng);
        ed.selection.collapse(true); // save a bookmark at the end of the list item

        var bookmark = ed.selection.getBookmark(); // copy the image an its text to the list item

        var clone = n.parentNode.cloneNode(true);
        if (clone.tagName === 'P' || clone.tagName === 'DIV') addChildren(clone, li);else li.appendChild(clone); // remove the old copy of the image

        n.parentNode.parentNode.removeChild(n.parentNode); // move the caret where we saved the bookmark
      ...
      function setCursorPositionToOriginalLi(li) {
        ...
        if (list != null) {
          var lastLi = list.lastChild; // Removed this line since IE9 would report an DOM character error and placing the caret inside an empty LI is handled and should be handled by the selection logic
      ...
      function fixListItem(parent, reference) {
        ...
        ed.selection.setCursorLocation(n, 0); // repaint to remove rendering artifact. only visible when creating new list
    ...
    applyList: function applyList(targetListType, oppositeListType) {
      var t = this,
          ed = t.ed,
          dom = ed.dom,
          applied = [],
      ...
      function makeList(element) {
        ...
        if (element.tagName === 'LI') {// No change required.
      ...
      function processBrs(element, callback) {
        ...
        function nextLeaf(br) {
          ...
          if (!dom.isBlock(br.parentNode) && br.parentNode !== dom.getRoot()) return nextLeaf(br.parentNode);
        } // Split on BRs within the range and process those.


        startSection = element.firstChild; // First mark the BRs that have any part of the previous section selected.
    ...
    outdent: function outdent(ui, elements) {
      var t = this,
          ed = t.ed,
          dom = ed.dom,
          outdented = [];
    ...
    process: function process(actions) {
      ...
      function isEmptyElement(element) {
        var excludeBrsAndBookmarks = tinymce.grep(element.childNodes, function (n) {
          return !(n.nodeName === 'BR' || n.nodeName === 'SPAN' && dom.getAttrib(n, 'data-mce-type') == 'bookmark' || n.nodeType == 3 && (n.nodeValue == String.fromCharCode(160) || n.nodeValue == ''));
        });
      ...
      function brAtEdgeOfSelection(container, offset) {
        return offset >= 0 && container.hasChildNodes() && offset < container.childNodes.length && container.childNodes[offset].tagName === 'BR';
      }
      ...
      bookmark = null; // we avoid doing repaint in a table as this will move the caret out of the table in Firefox 3.6
    ...
    splitSafeEach: function splitSafeEach(elements, f) {
    ...
    classBasedEach: function classBasedEach(elements, f) {
      var dom = this.ed.dom,
          nodes,
          element; // Mark nodes
    ...
    adjustPaddingFunction: function adjustPaddingFunction(isIndent) {
      var indentAmount,
          indentUnits,
          ed = this.ed;
