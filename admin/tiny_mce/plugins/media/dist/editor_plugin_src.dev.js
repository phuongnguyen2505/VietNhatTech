...
(function () {
  var rootAttributes = tinymce.explode('id,name,width,height,style,align,class,hspace,vspace,bgcolor,type'),
      ...
      mimeTypes; // Media types supported by this plugin

  mediaTypes = [// Type, clsid:s, mime types, codebase
  ...
  function toArray(obj) {
    ...
    if (obj && !obj.splice) {
      ...
      for (i = 0; true; i++) {
        if (obj[i]) out[i] = obj[i];else break;
      }
  ...
  tinymce.create('tinymce.plugins.MediaPlugin', {
    init: function init(ed, url) {
      ...
      ;
      self.editor = ed;
      self.url = url; // Parse media types into a lookup table
      ...
      tinymce.each(ed.getParam("media_types", "video=mp4,m4v,ogv,webm;" + "silverlight=xap;" + "flash=swf,flv;" + "shockwave=dcr;" + "quicktime=mov,qt,mpg,mpeg;" + "shockwave=dcr;" + "windowsmedia=avi,wmv,wm,asf,asx,wmx,wvx;" + "realmedia=rm,ra,ram;" + "java=jar;" + "audio=mp3,ogg").split(';'), function (item) {
        ...
        for (i = 0; i < extensions.length; i++) {
          ...
          if (type) lookup[extensions[i]] = type;
        }
      });
      ...
      ed.onPreInit.add(function () {
        ...
        ed.schema.addValidElements('object[id|style|width|height|classid|codebase|*],param[name|value],embed[id|style|width|height|type|src|*],video[*],audio[*],source[*]'); // Convert video elements to image placeholder
        ...
        ed.serializer.addNodeFilter('img', function (nodes, name, args) {
          ...
          while (i--) {
            node = nodes[i];
            if ((node.attr('class') || '').indexOf('mceItemMedia') !== -1) self.imgToObject(node, args);
          }
        });
      });
      ed.onInit.add(function () {
        ...
        if (ed.theme && ed.theme.onResolveName) {
          ed.theme.onResolveName.add(function (theme, path_object) {
            if (path_object.name === 'img' && ed.dom.hasClass(path_object.node, 'mceItemMedia')) path_object.name = 'media';
          });
        ...
        if (ed && ed.plugins.contextmenu) {
          ed.plugins.contextmenu.onContextMenu.add(function (plugin, menu, element) {
            if (element.nodeName === 'IMG' && element.className.indexOf('mceItemMedia') !== -1) menu.add({
              title: 'media.edit',
              icon: 'media',
              cmd: 'mceMedia'
            });
      ...
      ed.addCommand('mceMedia', function () {
        ...
        if (isMediaImg(img)) {
          ...
          if (data) {
            data = JSON.parse(data); // Add some extra properties to the data object
    ...
    convertUrl: function convertUrl(url, force_absolute) {
      var self = this,
          editor = self.editor,
      ...
      if (!url) return url;
      if (force_absolute) return editor.documentBaseURI.toAbsolute(url);
     ...
     */
    dataToImg: function dataToImg(data, force_absolute) {
      var self = this,
          editor = self.editor,
      ...
      if (attrs) attrs.src = self.convertUrl(attrs.src, force_absolute);
      if (attrs) attrs.poster = self.convertUrl(attrs.poster, force_absolute);
     ...
     */
    dataToHtml: function dataToHtml(data, force_absolute) {
     ...
     * Converts the JSON data object to a HTML string.
     */
    htmlToData: function htmlToData(html) {
      ...
      if (img) {
        ...
        data.type = this.getType(img.attr('class')).name.toLowerCase(); // Add some extra properties to the data object
    ...
    getType: function getType(value) {
      var i, values, typeItem; // Find type by checking the classes

      values = tinymce.explode(value, ' ');

      for (i = 0; i < values.length; i++) {
        typeItem = this.lookup[values[i]];
        if (typeItem) return typeItem;
      }
    },
     ...
     */
    imgToObject: function imgToObject(node, args) {
      var self = this,
          editor = self.editor,
          video,
          object,
      ...
      function addPlayer(video_src, poster_src) {
        ...
        if (flashPlayer) {
          baseUri = editor.documentBaseURI;
          data.params.src = flashPlayer; // Convert the movie url to absolute urls
          ...
          tinymce.each(flashVars, function (value, name) {
            ...
            if (value.length > 0) flashVarsOutput += (flashVarsOutput ? '&' : '') + name + '=' + escape(value);
          });
          if (flashVarsOutput.length) data.params.flashvars = flashVarsOutput;
      ...
      if (!style) {
        ...
        if (style) style = editor.dom.serializeStyle(editor.dom.parseStyle(style, 'img'));
      } // Handle iframe
      ...
      if (typeItem.name === 'Iframe') {
        ...
        tinymce.each(rootAttributes, function (name) {
          ...
          if (name == 'class' && value) value = value.replace(/mceItem.+ ?/g, '');
          if (value && value.length > 0) replacement.attr(name, value);
        });
      ...
      if (typeItem.name === 'Video' && data.video.sources[0]) {
        ...
        video = new Node('video', 1).attr(tinymce.extend({
          ...
          height: node.attr('height'),
          style: style
        }, data.video.attrs)); // Get poster source and use that for flash fallback

        if (data.video.attrs) posterSrc = data.video.attrs.poster;
        sources = data.video.sources = toArray(data.video.sources);

        for (i = 0; i < sources.length; i++) {
          if (/\.mp4$/.test(sources[i].src)) mp4Source = sources[i].src;
        }
      ...
      if (typeItem.name === 'EmbeddedAudio') {
        ...
        tinymce.each(rootAttributes, function (name) {
          if (data[name] && name != 'type') embed.attr(name, data[name]);
        });
        data.params.src = '';
      ...
      if (data.params.src) {
        ...
        if (/\.flv$/i.test(data.params.src)) addPlayer(data.params.src, '');
        if (args && args.force_absolute) data.params.src = editor.documentBaseURI.toAbsolute(data.params.src); // Create new object element
        ...
        tinymce.each(rootAttributes, function (name) {
          var value = data[name];
          if (name == 'class' && value) value = value.replace(/mceItem.+ ?/g, '');
          if (value && name != 'type') object.attr(name, value);
        }); // Add params

        for (name in data.params) {
          param = new Node('param', 1);
          param.shortEnded = true;
          value = data.params[name]; // Windows media needs to use url instead of src for the media URL

          if (name === 'src' && typeItem.name === 'WindowsMedia') name = 'url';
          param.attr({
            name: name,
            value: value
          });
      ...
      if (n) node.replace(n);else node.remove();
    },
     ...
     */
    objectToImg: function objectToImg(node) {
      var object,
          embed,
          video,
          iframe,
      ...
      if (!node.parent) return; // Handle media scripts

      if (node.name === 'script') {
        if (node.firstChild) matches = scriptRegExp.exec(node.firstChild.value);
        if (!matches) return;
        type = matches[1];
        data = {
          video: {},
      ...
      if (name === 'video' || name == 'audio') {
        ...
        if (source) data.video.sources.push({
          src: urlConverter.call(urlConverterScope, source, 'src', node.name)
        }); // Get all sources
        ...
        if (attrs.poster) attrs.poster = urlConverter.call(urlConverterScope, attrs.poster, 'poster', node.name);
      } // Object element
      ...
      if (node.name === 'embed') embed = node; // Iframe element
      ...
      if (object) {
        ...
        data.name = object.attr('name'); // Get all object params
        ...
        for (i = 0; i < params.length; i++) {
          ...
          if (!excludedAttrs[name]) data.params[name] = param.attr('value');
        }
      ...
      if (embed) {
        ...
        bgcolor = bgcolor || embed.attr('bgcolor'); // Get all embed attributes

        for (name in embed.attributes.map) {
          if (!excludedAttrs[name] && !data.params[name]) data.params[name] = embed.attributes.map[name];
        }
      }
      ...
      if (iframe) {
        ...
        for (name in iframe.attributes.map) {
          if (!excludedAttrs[name] && !data.params[name]) data.params[name] = iframe.attributes.map[name];
        }
      } // Use src not movie
      ...
      if (data.params.src) data.params.src = urlConverter.call(urlConverterScope, data.params.src, 'src', 'object');

      if (video) {
        if (node.name === 'video') type = lookup.video.name;else if (node.name === 'audio') type = lookup.audio.name;
      }

      if (object && !type) type = (lookupAttribute(object, 'clsid') || lookupAttribute(object, 'classid') || lookupAttribute(object, 'type') || {}).name;
      if (embed && !type) type = (lookupAttribute(embed, 'type') || lookupExtension(data.params.src) || {}).name; // for embedded audio we preserve the original specified type
      ...
      node.replace(img); // Remove embed

      if (embed) embed.remove(); // Serialize the inner HTML of the object element

      if (object) {
        html = getInnerHTML(object.remove());
        if (html) data.object_html = html;
      } // Serialize the inner HTML of the video element


      if (video) {
        html = getInnerHTML(video.remove());
        if (html) data.video_html = html;
      }

      data.hspace = hspace;
      data.vspace = vspace;
      data.align = align;
      data.bgcolor = bgcolor; // Set width/height of placeholder
