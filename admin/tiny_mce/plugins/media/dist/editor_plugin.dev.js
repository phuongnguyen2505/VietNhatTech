...
(function () {
  var d = tinymce.explode("id,name,width,height,style,align,class,hspace,vspace,bgcolor,type"),
      h = tinymce.makeMap(d.join(",")),
      b = tinymce.html.Node,
      f,
      a,
      g = tinymce.util.JSON,
  ...
  tinymce.create("tinymce.plugins.MediaPlugin", {
    ...
    convertUrl: function convertUrl(k, n) {
      var j = this,
          m = j.editor,
          l = m.settings,
          o = l.url_converter,
          i = l.url_converter_scope || j;
    ...
    dataToImg: function dataToImg(m, k) {
      var r = this,
          o = r.editor,
          p = o.documentBaseURI,
          j,
    ...
    dataToHtml: function dataToHtml(i, j) {
      return this.editor.serializer.serialize(this.dataToImg(i, j), {
        forced_root_block: "",
        force_absolute: j
      });
    },
    htmlToData: function htmlToData(k) {
      var j, i, l;
      l = {
        type: "flash",
        video: {
    ...
    imgToObject: function imgToObject(z, o) {
      var u = this,
          p = u.editor,
          C,
          H,
      ...
      function r(i, n) {
        var M, L, N, K, J;
        J = p.getParam("flash_video_player_url", u.convertUrl(u.url + "/moxieplayer.swf"));
        ...
        if (J) {
          ...
          if (p.getParam("flash_video_player_absvideourl", true)) {
            i = M.toAbsolute(i || "", true);
            n = M.toAbsolute(n || "", true);
          }

          N = "";
          L = p.getParam("flash_video_player_flashvars", {
            url: "$url",
            poster: "$poster"
          });
          tinymce.each(L, function (P, O) {
            P = P.replace(/\$url/, i || "");
            P = P.replace(/\$poster/, n || "");
          ...
          if (N.length) {
            G.params.flashvars = N;
          }

          K = p.getParam("flash_video_player_params", {
            allowfullscreen: true,
            allowscriptaccess: true
          });
          tinymce.each(K, function (P, O) {
            G.params[O] = "" + P;
          });
        }
      }
      ...
      if (this.editor.settings.media_use_script) {
        x = new b("script", 1).attr("type", "text/javascript");
        y = new b("#text", 3);
        y.value = "write" + q.name + "(" + g.serialize(tinymce.extend(G.params, {
          width: z.attr("width"),
      ...
      if (q.name === "Video" && G.video.sources[0]) {
        C = new b("video", 1).attr(tinymce.extend({
          id: z.attr("id"),
          width: z.attr("width"),
          height: z.attr("height"),
          style: B
      ...
      if (q.name === "Audio" && G.video.sources[0]) {
        F = new b("audio", 1).attr(tinymce.extend({
          id: z.attr("id"),
          width: z.attr("width"),
          height: z.attr("height"),
          style: B
    ...
    objectToImg: function objectToImg(C) {
      var L,
          k,
          F,
          s,
          ...
          m,
          z,
          v = this.editor.settings.url_converter,
      ...
      function J(O, i) {
        return H[(O.attr(i) || "").toLowerCase()];
      }

      function p(O) {
        var i = O.replace(/^.*\.([^.]+)$/, "$1");
        return H[i.toLowerCase() || ""];
      }
