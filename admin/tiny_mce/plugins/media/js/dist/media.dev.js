...
(function () {
  ...
  function clone(obj) {
    var i, len, copy, attr;
    if (null == obj || "object" != _typeof(obj)) return obj; // Handle Array
  ...
  function setVal(id, value, name) {
    if (typeof value != 'undefined' && value != null) {
      var elm = get(id);
      if (elm.nodeName == "SELECT") selectByValue(document.forms[0], id, value);else if (elm.type == "checkbox") {
  ...
  window.Media = {
    init: function init() {
      var html,
          editor,
          self = this;
      self.editor = editor = tinyMCEPopup.editor; // Setup file browsers and color pickers
      ...
      if (html == "") get("linklistrow").style.display = 'none';else get("linklistcontainer").innerHTML = html;
      if (isVisible('filebrowser')) get('src').style.width = '230px';
      if (isVisible('video_filebrowser_altsource1')) get('video_altsource1').style.width = '220px';
      if (isVisible('video_filebrowser_altsource2')) get('video_altsource2').style.width = '220px';
      if (isVisible('audio_filebrowser_altsource1')) get('audio_altsource1').style.width = '220px';
      if (isVisible('audio_filebrowser_altsource2')) get('audio_altsource2').style.width = '220px';
      if (isVisible('filebrowser_poster')) get('video_poster').style.width = '220px';
    ...
    moveStates: function moveStates(to_form, field) {
      var data = this.data,
          editor = this.editor,
      ...
      function setOptions(type, names) {
        ...
        if (type == data.type || type == 'global') {
          ...
          for (i = 0; i < names.length; i++) {
            ...
            if (type == 'global') list = data;else if (type == 'video' || type == 'audio') {
            ...
            if (list) {
              if (to_form) {
                ...
                if ((type == 'video' || type == 'audio') && value === true) value = name;
      ...
      if (!to_form) {
        ...
        data.height = getVal('height'); // Switch type based on extension

        src = getVal('src');

        if (field == 'src') {
          ext = src.replace(/^.*\.([^.]+)$/, '$1');
          if (typeInfo = mediaPlugin.getType(ext)) data.type = typeInfo.name.toLowerCase();
      ...
      if (get(data.type + '_options')) get(data.type + '_options').style.display = 'block';
      setVal('media_type', data.type);
      ...
      if (to_form) {
        if (data.type == 'video') {
          ...
          if (data.type == 'flash') {
            tinymce.each(editor.getParam('flash_video_player_flashvars', {
              ...
              if (value == '$url') data.params.src = parseQueryParams(data.params.flashvars)[name] || data.params.src || '';
            });
          }
        ...
        if (data.type == 'video') {
          ...
          if (src) data.video.sources[1] = {
            src: src
          };
          src = getVal("video_altsource2");
          if (src) data.video.sources[2] = {
            src: src
          };
        } else if (data.type == 'audio') {
          if (!data.video.sources) data.video.sources = [];
        ...
        } else data.params.src = src; // Set default size
    ...
    dataToForm: function dataToForm() {
      this.moveStates(true);
    },
    formToData: function formToData(field) {
    ...
    getMediaListHTML: function getMediaListHTML() {
    ...
    getMediaTypeHTML: function getMediaTypeHTML(editor) {
    ...
    setDefaultDialogSettings: function setDefaultDialogSettings(editor) {
