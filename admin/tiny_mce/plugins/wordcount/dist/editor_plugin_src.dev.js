  ...
  tinymce.create('tinymce.plugins.WordCount', {
    ...
    init: function init(ed, url) {
      ...
      ed.onPostRender.add(function (ed, cm) {
        var row, id; // Add it to the specified id or the theme advanced path

        id = ed.getParam('wordcount_target_id');

        if (!id) {
          row = tinymce.DOM.get(ed.id + '_path_row');
          if (row) tinymce.DOM.add(row.parentNode, 'div', {
            'style': 'float: right'
    ...
    _getCount: function _getCount(ed) {
      var tc = 0;
    ...
    _count: function _count(ed) {
      var t = this; // Keep multiple calls from happening at the same time
