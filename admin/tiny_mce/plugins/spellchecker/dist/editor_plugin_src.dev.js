  ...
  tinymce.create('tinymce.plugins.SpellcheckerPlugin', {
    ...
    init: function init(ed, url) {
      ...
      if (t.rpcUrl == '{backend}') {
        ...
        t.hasSupport = true; // Disable the context menu when spellchecking is active
        ...
        if (!t.active) {
          ...
          t._sendRPC('checkWords', [t.selectedLang, t._getWords()], function (r) {
            if (r.length > 0) {
              ...
              ed.setProgressState(0);
              if (ed.getParam('spellchecker_report_no_misspellings', true)) ed.windowManager.alert('spellchecker.no_mpell');
            }
          });
        } else t._done();
      });
      if (ed.settings.content_css !== false) ed.contentCSS.push(url + '/css/content.css');
    ...
    createControl: function createControl(n, cm) {
      ...
      if (n == 'spellchecker') {
        ...
        if (t.rpcUrl == '{backend}') {
          ...
          if (t.hasSupport) c = cm.createButton(n, {
    ...
    _getSeparators: function _getSeparators() {
      var re = '',
          i,
          str = this.editor.getParam('spellchecker_word_separator_chars', "\\s!\"#$%&()*+,-./:;<=>?@[]^_{|}\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\u201D\u201C"); // Build word separator regexp
    ...
    _getWords: function _getWords() {
      var ed = this.editor,
          wl = [],
          tx = '',
          lo = {},
          rawWords = []; // Get area text
    ...
    _removeWords: function _removeWords(w) {
      var ed = this.editor,
          dom = ed.dom,
          se = ed.selection,
          r = se.getRng(true);
    ...
    _markWords: function _markWords(wl) {
      var ed = this.editor,
          dom = ed.dom,
          doc = ed.getDoc(),
          se = ed.selection,
          ...
          rx = new RegExp('(^|[' + re + '])(' + w + ')(?=[' + re + ']|$)', 'g'); // Collect all text nodes
      ...
      each(nl, function (n) {
        ...
        if (rx.test(v)) {
          // Encode the content
          v = dom.encode(v); // Create container element
          ...
          if (tinymce.isIE) {
            ...
            v = v.replace(rx, '$1<mcespell>$2</mcespell>'); // Loop over the content finding mispelled words
            ...
            while ((pos = v.indexOf('<mcespell>')) != -1) {
              ...
              txt = v.substring(0, pos);
              v = v.substring(pos + 11); // Add span element for the word
    ...
    _showMenu: function _showMenu(ed, e) {
      var t = this,
          ed = t.editor,
          m = t._menu,
          p1,
          ...
          if (ed.getParam('show_ignore_words', true)) {
            ...
            m.add({
              ...
              onclick: function onclick() {
                ...
                t._checkDone(); // tell the server if we need to
    ...
    _checkDone: function _checkDone() {
      var t = this,
          ed = t.editor,
          dom = ed.dom,
          o;
    ...
    _sendRPC: function _sendRPC(m, p, cb) {
      var t = this;
      JSONRequest.sendRPC({
        url: t.rpcUrl,
        method: m,
