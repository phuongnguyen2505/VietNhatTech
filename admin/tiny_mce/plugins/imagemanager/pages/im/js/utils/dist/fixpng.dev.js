...
var PNG = {
  ...
  fix: function fix(e) {
    var rs, cs, b, o; // Remove behavior to prevent memory leaks

    e.runtimeStyle.behavior = "none"; // Check browser version

    if (!this.isOldIE) return; // Use old PNG src

    if (e.png_src) e.src = e.png_src; // Is PNG image
    ...
    cs = e.currentStyle;
    b = cs.backgroundImage; // Has PNG background

    if (b && b.toLowerCase().indexOf('png') > 0) {
      rs = e.runtimeStyle;
      b = b.replace(/url\(\"([^"]+)\"\)/g, "$1");
      o = rs["background-position"];
      if (!cs.hasLayout) rs.display = 'inline-block';
