  ...
  window.RPC = {
    ...
    init: function init() {
      $().ajaxError(function (e, req, se) {
        if (req.status) $.WindowManager.status({
          title: "The server response wasn't JSON format",
          content: req.responseText
        });
      });
    },
    toArray: function toArray(res) {
      var fl = [];
    ...
    handleError: function handleError(args) {
      var t = this,
          errors,
          res = args.response.result,
          err = args.response.error;

      if (err) {
        if (err.level == 'AUTH') {
          if (res.login_url.indexOf('return_url') != -1) document.location = res.login_url;else if (res.login_url.indexOf('://') == -1) document.location = "../../" + res.login_url + "?return_url=" + escape(document.location);else document.location = res.login_url + "?return_url=" + escape(document.location);
          return true;
        }
        ...
        $(this.toArray(res)).each(function (i, r) {
          ...
          if (r.status && r.status.toLowerCase() != 'ok') {
            ...
            if (r.fromfile != undefined) {
              ...
              errors.push({
                title: r.fromfile + " -> " + r.tofile,
                content: $.translate(r.message)
              });
            }
    ...
    insertFiles: function insertFiles(s) {
      var t = this,
          args = {};
      $(s.paths).each(function (i, v) {
        args['path' + i] = v;
      });
      if (s.progress_message) $.WindowManager.showProgress({
    ...
    insertFile: function insertFile(s) {
      var ins = s.oninsert;
      s.paths = [s.path];
