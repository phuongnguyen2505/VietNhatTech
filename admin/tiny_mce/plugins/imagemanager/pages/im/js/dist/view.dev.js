  ...
  window.ViewDialog = {
    ...
    init: function init() {
      ...
      t.args = args = $.extend({
        path: '{default}',
        visual_path: '/'
      }, t.currentWin.getArgs()); // Compile templates
      ...
      $('#prev').click(function (e) {
        if (!$(e.target).hasClass('disabled')) t.getMediaInfo(t.prevMedia);
      });
      $('#next').click(function (e) {
        if (!$(e.target).hasClass('disabled')) t.getMediaInfo(t.nextMedia);
      });
      $('#gallery').click(function (e) {
        if (!$(e.target).hasClass('disabled')) t.currentWin.close();
      });
      $('#singlefooter').click(function (e) {
        var el = e.target,
            a = el.nodeName == 'A' ? el : $(el).parents('a')[0];

        if (a) {
          a = $(a);
          if (a.attr('id') == 'deleteit' && !a.hasClass('disabled')) t.deleteFile(t.path);
      ...
      $('#singlecontent').click(function (e) {
        if (e.target.nodeName == 'IMG' && $(e.target).hasClass('viewimage')) {
          ...
          } else t.currentWin.close();
        }
      });
      t.getMediaInfo(args.path);
      focus();
    },
    getMediaInfo: function getMediaInfo(p) {
      var t = this;
      t.path = p;
      RPC.exec('im.getMediaInfo', {
        ...
        if (!row.next) $('#next').addClass('disabled');else $('#next').removeClass('disabled');
        if (!row.prev) $('#prev').addClass('disabled');else $('#prev').removeClass('disabled');
        ...
        if (row.custom) {
          if (!row.custom.editable) $('#singleview .editsingle a.edt').addClass('disabled');else $('#singleview .editsingle a.edt').removeClass('disabled');
        }
          ...
          if (!t.isDemo()) {
            RPC.exec('im.deleteFiles', {
              ...
              if (!RPC.handleError({
                ...
                if (t.args.ondelete) t.args.ondelete(p);
                if (t.nextMedia) t.getMediaInfo(t.nextMedia);else if (t.prevMedia) t.getMediaInfo(t.prevMedia);else t.currentWin.close();
              }
            });
          }
        }
