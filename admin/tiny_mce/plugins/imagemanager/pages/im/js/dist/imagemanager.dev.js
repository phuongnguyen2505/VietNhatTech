...
(function ($) {
  window.ImageManager = $.extend(BaseManager, {
    ...
    pageSize: 25,
    viewMode: 'thumbs',
    tools: ['createdir', 'preview', 'refresh', 'edit', 'upload', 'rename', 'delete', 'download', 'insert', 'filemanager', 'help'],
    init: function init() {
      var t = this,
          args = t.currentWin.getArgs(),
          uri; // Compile templates
      ...
      t.urlSuffix = '';
      if (document.domain != document.location.hostname) t.urlSuffix = '?domain=' + document.domain;

      if (args.url) {
        uri = $.parseURI(args.url, {
          base_url: args.document_base_url || args.default_base_url
        });
        if (uri) t.inputURL = uri.path.replace(/\/[^\/]+$/, '');
      }
      ...
      $.extend($.template.helpers, {
        add: function add(v, a) {
          if (v != 'auto') v = parseInt(v) + parseInt(a) + 'px';
          return v;
        }
      });
      t.menu = new $.mcDropMenu({
        halign: 'right',
        valign: 'bottom',
        setup: function setup(m) {
          $(m).bind('DropMenu:beforeshow', function (e, m) {
            var file = t.focusedFile; // Build new menu

            m.clear();
            if (file.type != 'folder') m.add({
              title: $.translate('{#common.insert}'),
              disabled: t.isDisabled('insert'),
              onclick: function onclick() {
                t.insertFiles();
              }
            });
            m.add({
              title: $.translate('{#common.deleteit}'),
              disabled: t.isDisabled('delete'),
              onclick: function onclick() {
                t.deleteFiles([file]);
              }
            });
            ...
            if (file.type != 'folder') {
              if (file.custom.editable) m.add({
                title: $.translate('{#common.edit}'),
                disabled: t.isDisabled('edit'),
                onclick: function onclick() {
                  t.editFile(file);
                }
              });
      ...
      t.menu.render('#filelist'); // Register toolbar actions

      $().bind('action:createdir', function () {
        t.createDir();
      });
      $().bind('action:upload', function () {
        t.uploadFiles();
      });
      $().bind('action:refresh', function () {
        t.listFiles();
        t.listDirs();
      });
      $().bind('action:help', function () {});
      $().bind('action:filemanager', function () {
        t.fileManager();
      });
      $('#tools li').each(function (i, v) {
        $(v).click(function (e) {
          if (!$(v).hasClass('disabled')) $().trigger('action:' + v.id, e);
          e.preventDefault();
          return false;
        });
      });
      $('#filelist').click(function (e) {
        var a = $(e.target).parents('a')[0],
            fo,
            o,
            div,
            img;
      ...
      $('#pages').click(function (e) {
        ...
        if (el.nodeName == 'A') {
          ...
          if ($(el).hasClass('next')) t.page++;else if ($(el).hasClass('prev')) t.page--;
          t.listFiles();
          e.preventDefault();
          return false;
        }
      ...
      function filter(e) {
        var f = $(e.target).val();
        if (f == t.filterVal) return;
        t.filterVal = f;
        if (f == '') f = null;else if (f.indexOf('*') == -1) f = '*' + f + '*';
        t.page = 0;
        t.filter = f;
        t.listFiles();
        t.listDirs();
    ...
    listFiles: function listFiles(p) {
      var t = this,
          args = t.currentWin.getArgs();
      t.path = p || t.path;
      $('#pages').hide();
      ...
      if (t.pages) t.page = t.page > t.pages - 1 ? t.pages - 1 : t.page;
      RPC.exec('im.listFiles', {
        ...
        if (!RPC.handleError({
          ...
          visual_path: t.visualPath,
          response: data
        })) {
          var explode = function explode(s) {
          ...
          if (argDisabledTools = explode(args.disabled_tools)) disabled = jQuery.merge(argDisabledTools, disabled);

          if (argTools = explode(args.tools)) {
            $(argTools).each(function (i, v) {
              if (!$.inArray(v, visible)) visible.push(v);
            });
          ...
          $(t.tools).each(function (i, v) {
            ...
            if ($.inArray(v, visible) != -1) li.show();else li.hide();
          });
          $('#tools').show();
          $('#progress').hide();
          $('#curpath').html(t.visualPath).attr('title', t.visualPath); // Convert result table into object list

          t.files = RPC.toArray(data.result); // Update file list
    ...
    updateFileList: function updateFileList() {
      ...
      $('#pages').show();
      if (!t.page) $('#pages .prev').addClass('disabled');else $('#pages .prev').removeClass('disabled');
      if (t.page == t.pages - 1) $('#pages .next').addClass('disabled');else $('#pages .next').removeClass('disabled');
    ...
    deleteFiles: function deleteFiles(fl) {
      var t = this,
          args = {};
        ...
        oninsert: function oninsert(o) {
          ...
          if (s.oninsert) {
            $(o.files).each(function (i, v) {
              if (v.path == t.focusedFile.path) o.focusedFile = v;
            });
            s.oninsert(o);
          }
        }
    ...
    fileManager: function fileManager() {
      var suf;
      if (this.fileManagerURL.indexOf('?') != -1) suf = this.urlSuffix.replace(/\?/, '&');
    ...
    listRoots: function listRoots() {
      ...
      if (t.rootPathName) {
        $('#category_list').html(t.caregoryListTpl, {
          name: t.rootPathName,
          path: t.rootPath
        });
        return;
