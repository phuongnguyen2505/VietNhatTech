  ...
  window.CreateDirDialog = {
    ...
    init: function init() {
      ...
      args = $.extend({
        path: '{default}',
        visual_path: '/'
      }, t.currentWin.getArgs()); // Add templates
      ...
      RPC.exec('im.getConfig', {
        ...
        if (!RPC.handleError({
          ...
          if (config['filesystem.force_directory_template'] == "true") {
            $('#template').html('');
            if (templates.length == 1) $('#templaterow').hide();
          }

          if (templates.length == 0) $('#templaterow').hide();
          ...
          $('form').submit(function () {
            var tpl = $('#template').val(),
                name = $('#dirname').val();
            if (cleanNames) name = $.cleanName(name);

            if (!t.isDemo()) {
              RPC.exec('im.createDirs', {
                path: args.path,
                name0: name,
                template0: tpl
              }, function (data) {
                ...
                if (!RPC.handleError({
                  message: '{#error.createdir_failed}',
