...
(function ($) {
  window.EditDialog = {
      ...
      t.apply = function () {
        ...
        RPC.exec('im.getConfig', {
          ...
          if (!RPC.handleError({
            ...
            visual_path: t.args.visual_path,
            response: data
          })) {
            if (config['filesystem.clean_names'] == "true") $('#save_filename').val($.cleanName($('#save_filename').val()));
            $.WindowManager.showProgress({
              message: $.translate("{#edit_image.saving_wait}")
            });
            RPC.exec('im.saveImage', {
              path: t.imgPath,
              target: $('#save_filename').val()
            }, function (data) {
              var res;
              $.WindowManager.hideProgress();
              ...
              if (!RPC.handleError({
                ...
                if (t.args.onsave) return t.insertFile(res[0].file);
                if (res.length > 0) t.loadImage({
                  path: res[0].file,
                  initial: 1
                });
    ...
    resize: function resize() {
      ...
      $('#resize_tools input[@type=text]').change(function (e) {
        if (f.resize_prop.checked) {
          if (e.target.id == "resize_w") f.resize_h.value = Math.round(t.imageSelection.h * (parseInt(f.resize_w.value) / t.imageSelection.w));else f.resize_w.value = Math.round(t.imageSelection.w * (parseInt(f.resize_h.value) / t.imageSelection.h));
        }
      ...
      t.apply = function () {
        $.WindowManager.showProgress({
          message: $.translate('{#edit_image.please_wait}')
        });
        t.execRPC('im.resizeImage', {
          path: t.imgPath,
          width: f.resize_w.value,
          height: f.resize_h.value,
          temp: true
        }, '{#error.resize_failed}');
      };
    },
    crop: function crop() {
      ...
      $('#crop_tools input[@type=text]').change(function (e) {
        if (f.crop_prop.checked) {
          if (e.target.id == "crop_w") f.crop_h.value = Math.round(t.imageSelection.h * (parseInt(f.crop_w.value) / t.imageSelection.w));else f.crop_w.value = Math.round(t.imageSelection.w * (parseInt(f.crop_h.value) / t.imageSelection.h));
        }
      ...
      t.apply = function () {
        ...
        t.execRPC('im.cropImage', {
          path: t.imgPath,
          left: f.crop_x.value,
          top: f.crop_y.value,
          width: f.crop_w.value,
          height: f.crop_h.value,
          temp: true
        }, '{#error.crop_failed}');
      };
    },
    flip: function flip() {
      ...
      $(t.imgUtils).bind('ImageUtils:load', function () {
        ...
        t.apply = function () {
          ...
          if (axis) t.execRPC('im.flipImage', {
            path: t.imgPath,
            horizontal: axis == 'h',
            vertical: axis == 'v',
            temp: true
          }, '{#error.flip_failed}');else t.cancel();
        };
      });
      t.imgUtils.render();
    },
    rotate: function rotate() {
      ...
      $(t.imgUtils).bind('ImageUtils:load', function () {
        ...
        t.apply = function () {
          ...
          if (ang) t.execRPC('im.rotateImage', {
            path: t.imgPath,
            angle: ang,
            temp: true
          }, '{#error.rotate_failed}');else t.cancel();
        };
      });
      t.imgUtils.render();
    },
    execRPC: function execRPC(m, a, er) {
      var t = this;
    ...
    insertFile: function insertFile(p) {
      var t = this,
          s = t.args;
      RPC.insertFile({
    ...
    loadImage: function loadImage(o, cb) {
      var t = this;
      $('#crop,#resize,#flip,#rotate').addClass('disabled');
      $.WindowManager.showProgress({
        message: $.translate(o.initial ? "{#edit_image.loading}" : "{#edit_image.please_wait}")
      });
      RPC.exec('im.getMediaInfo', {
        path: o.path,
        url: o.url
      }, function (data) {
        var res = RPC.toArray(data.result);
        ...
        if (!RPC.handleError({
          ...
          if (o.initial) t.imageURL = res[0].url; // Initial load

          if (o.initial) t.targetPath = res[0].path;
    ...
    resizeView: function resizeView() {
      $('#imageWrapper').css({
        'width': (window.innerWidth || $.winWidth()) - 30,
        'height': (window.innerHeight || $.winHeight()) - 100
      });
    },
