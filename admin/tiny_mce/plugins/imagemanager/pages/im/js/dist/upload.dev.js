...
(function ($) {
  window.UploadDialog = {
    ...
    init: function init() {
      ...
      $('.uploadtype').html($.translate('{#upload.basic_upload}', 0, {
        a: '<a id="singleupload" href="#basic">',
        '/a': '</a>'
      }));
      ...
      $('form').submit(function () {
        $.WindowManager.showProgress({
          message: $.translate('{#upload.progress}')
        });
      });
      if (document.location.hostname != document.domain) $('form input[name=domain]').val(document.domain);
      t.path = args.path;
      ...
      RPC.exec('im.getConfig', {
        ...
        $('#content').show();
        if ($.multiUpload.initialized) $('#multiupload_view').show();else $('#singleupload_view').show(); // Disabled upload
        ...
        if (upExt[0] == '*') upExt = fsExt;
        if (fsExt[0] == '*') fsExt = upExt;
        ...
        for (i = 0; i < upExt.length; i++) {
          ...
          if (found) outExt.push(upExt[i]);
        }

        t.validExtensions = outExt;
        t.maxSize = maxSize;
        $('#facts').html($.templateFromScript('#facts_template'), {
          extensions: outExt.join(', '),
          maxsize: maxSize,
          path: args.visual_path
        });
        if (config['upload.multiple_upload'] == "true") t.initMultiUpload();
      });
      $('#cancel').click(function () {
        t.currentWin.close();
      });
    },
    cleanName: function cleanName(s) {
      if (this.shouldCleanNames) s = $.cleanName(s);
      return s;
    },
    handleSingleUploadResponse: function handleSingleUploadResponse(data) {
      var t = this,
    ...
    initMultiUpload: function initMultiUpload() {
      var t = this,
          up,
          args = t.currentWin.getArgs(),
          initial = 1,
      ...
      if (t.debug) alert('Runtime used: ' + $.multiUpload.runtime);
      ...
      function calc(up) {
        ...
        $(up.files).each(function (i, f) {
          size += f.size;
          loaded += f.loaded;
          if (f.status == 'completed') uploaded++;
          if (!f.status) finished = false;
        });
        ...
        if (finished) {
          $('#abortupload').hide();
          $(up.files).each(function (i, f) {
            if (f.status == 'completed') fl.push(t.path + '/' + f.name);
          });
          t.insertFiles(fl);
        ...
        $('#progressinfo').html($.translate('{#upload.progressinfo}', 1, {
          loaded: up.formatSize(loaded),
          total: up.formatSize(size),
          speed: up.formatSize(bps)
        }));
        $('#progressbar').css('width', Math.round(loaded / size * 100.0) + '%');
        $('#stats').html($.translate('{#upload.statusrow}', 1, {
          files: up.files.length,
          size: up.formatSize(size)
        }));
      }
      ...
      $(up).bind('multiUpload:filesSelected', function (e, fs) {
        ...
        $(fs.files).each(function (i, fo) {
          ...
          $('#files').show();
          $('#files tbody').append(t.fileListTpl, {
            id: fo.id,
            name: fo.name,
            size: fo.size
          });
      ...
      $(up).bind('multiUpload:chunkUploaded', function (e, o) {
        var res = $.parseJSON(o.response),
            data = RPC.toArray(res.result);
      ...
      $(up).bind('multiUpload:uploadChunkError', function (e, o) {
        $('#' + o.file.id).addClass('failed');
        $('#' + o.file.id + ' td.status').html('Failed').attr('title', o.error); //top.console.log(o.file, o.chunk, o.chunks, o.error);
      }); // Add UI events
    ...
    insertFiles: function insertFiles(pa) {
      var s = this.currentWin.getArgs(); // Insert file
