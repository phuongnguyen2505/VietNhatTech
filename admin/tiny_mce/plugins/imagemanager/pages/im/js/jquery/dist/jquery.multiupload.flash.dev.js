  ...
  if (!$.multiUpload.initialized && getFlashVersion() >= 10) {
    ...
    $.extend($.multiUpload.prototype, {
      ...
      repaint: function repaint() {
        var up = this,
            fb = $(up.settings.flash_browse_button),
            off = fb.offset();
        up.flashContainer.css({
          position: 'absolute',
          top: off.top,
          left: off.left,
          width: fb.width(),
      ...
      init: function init() {
        ...
        up.flashContainer = $('#flashUploaderContainer'); // Reposition flash

        $(function (e) {
          up.repaint();
        });
        up.repaint(); // Register event handlers
        ...
        $(up).bind('multiUpload:flashUploadChunkComplete', function (e, o) {
          ...
          if (!fo.status) $(up).trigger('multiUpload:chunkUploaded', [arg]);
