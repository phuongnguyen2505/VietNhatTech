...
(function ($) {
  if (!$.multiUpload.initialized && window.google && google.gears) {
    ...
    $.multiUpload.runtime = 'gears'; // Override init function
    ...
    $.extend($.multiUpload.prototype, {
      init: function init() {
        ...
        $(up).bind('multiUpload:selectFiles', function (e) {
          var up = this,
              desk = google.gears.factory.create('beta.desktop'),
              s = {};
          if (up.settings.filter[0] != '*') s.filter = $.map(up.settings.filter, function (v) {
            return '.' + v;
          });
          desk.openFiles(function (f) {
            ...
            if (f.length) {
              ...
              up._fireEvent('multiUpload:filesSelected', [{
                files: sf
              }]);

              up._fireEvent('multiUpload:filesChanged');
            } else up._fireEvent('multiUpload:filesSelectionCancelled');
          }, s);
        });
        $(up).bind('multiUpload:uploadFile', function (e, fo) {
          ...
          function uploadNextChunk() {
            ...
            req.upload.onprogress = function (pr) {
              ...
              up._fireEvent('multiUpload:fileUploadProgress', [{
                file: fo,
                loaded: fo.loaded,
                total: fo.size
              }]);
              ...
              if (req.readyState == 4) {
                if (req.status == 200) {
                  ...
                  if (++chunk >= chunks) {
                    ...
                    up._fireEvent('multiUpload:fileUploaded', [{
                      file: fo,
                      response: req.responseText
                    }]);
                ...
                } else up._fireEvent('multiUpload:uploadChunkError', [{
                  file: fo,
                  chunk: chunk,
                  chunks: chunks,
            ...
            if (chunk < chunks) req.send(fo.blob.slice(chunk * chunkSize, curChunkSize));
          }

          ;
        });
      },
      _fireEvent: function _fireEvent(ev, ar) {
        $(this).trigger(ev, ar);
      }
    });
  }
