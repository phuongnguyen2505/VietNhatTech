...
/**
 * $Id: jquery.dialog.js 589 2008-11-13 11:15:53Z spocke $
 *
 * @author Moxiecode
...
(function ($) {
  $.Window = function (f, a) {
    ...
    t.features = f;
    t.args = a;
    t.isIE6 = /MSIE [56]/.test(navigator.userAgent); // Register actions
  ...
  $.extend($.Window.prototype, {
    ...
    focus: function focus() {
      ...
      if (!t.focused) {
        if (t.shim) t.shim.css('z-index', $.WindowManager.zIndex);
    ...
    close: function close(ac) {
      var t = this,
          f;

      if (ac && (f = t.features.onbeforeclose)) {
        if (f.call(t) === false) return;
      }

      $('#' + t.id).remove();
      if ($.WindowManager.windows.length == 1) $('#windowManEventBlocker').remove();
      ...
      if (t.features.onclose) t.features.onclose(t);
    },
    ...
    resizeTo: function resizeTo(w, h, nr) {
      var t = this,
          b = t.borders,
          el = $('#' + t.id),
          ifr = $('#' + t.id + '_content, #' + t.id + '_ifr');
      w = parseInt(w);
      h = parseInt(h);
    ...
    setContent: function setContent(h) {
      $('#' + this.id + '_content').html(h);
      return this;
    },
    setURL: function setURL(u) {
      var t = this,
          id = t.id,
          ifr = $('#' + id + '_ifr'),
          b = t.borders;
      if (!ifr[0]) $('#' + id + '_content').html('').append($.createElm('iframe', {
        id: id + '_ifr',
        src: u,
        frameBorder: '0',
        scrolling: t.features.scrolling
      }).css({
        width: t.width - b.w,
        height: t.height - b.h
      }));else ifr.attr('src', u);
    ...
    startMove: function startMove(a, e) {
      var t = this,
          bid = t.features.theme + '_move',
          mb,
          sx = e.screenX,
          sy = e.screenY,
          dx,
          ...
          wx,
          wy;
      $(document.body).append($.createElm('div', {
        id: bid,
        'class': t.features.theme + '_event_blocker'
      }).css('z-index', $.WindowManager.zIndex));
      mb = $('#' + bid).css({
        width: $.winWidth(),
        height: $.winHeight(),
    ...
    startDrag: function startDrag(a, e) {
      var t = this,
          bid = t.features.theme + '_move',
          mb,
          sx = e.screenX,
    ...
    render: function render() {
      ...
      if (f.x == -1) f.x = Math.round($.winWidth() / 2 - f.width / 2);
      if (f.y == -1) f.y = Math.round($.winHeight() / 2 - f.height / 2);
      t.id = id;
      ...
      if (f.modal) {
        ...
        if (!bl[0]) {
          $(document.body).append($.createElm('div', {
            id: 'windowManEventBlocker',
            'class': f.theme + '_event_blocker'
          }).addClass(f.theme + '_visible_event_blocker'));
          bl = $('#windowManEventBlocker');
        }

        bl.show().css('z-index', $.WindowManager.zIndex++).css({
          width: $.winWidth(),
          height: $.winHeight(),
          position: t.isIE6 ? 'absolute' : 'fixed'
        });
      ...
      $(document.body).appendAll(['div', {
        id: id,
        'class': f.theme + ' window'
      }, ['div', {
        id: id + '_container',
        'class': 'statusbar ' + f.type
      }, ['div', {
        id: id + '_top',
        'class': 'windowtop'
      }, ['div', {
        ...
        id: id + '_resize_n',
        'class': 'action resize resize-n',
        tabindex: '-1'
      }], ['a', {
        id: id + '_resize_nw',
        'class': 'action resize resize-nw',
        tabindex: '-1'
      }], ['a', {
        id: id + '_resize_ne',
        'class': 'action resize resize-ne',
        tabindex: '-1'
      }]], ['div', {
        id: id + '_middle',
        'class': 'windowmiddle'
        ...
        id: id + '_resize_w',
        'class': 'action resize resize-w',
        tabindex: '-1'
      }], ['a', {
        id: id + '_resize_e',
        'class': 'action resize resize-e',
        tabindex: '-1'
      }]], ['div', {
        id: id + '_bottom',
        'class': 'windowbottom' + (f.statusbar ? ' statusbarbottom' : '')
      }, ['div', {
        'class': 'left'
      }], ['div', {
        'class': 'middle'
        ...
        id: id + '_resize_s',
        'class': 'action resize resize-s',
        tabindex: '-1'
      }], ['a', {
        id: id + '_resize_sw',
        'class': 'action resize resize-sw',
        tabindex: '-1'
      }], ['a', {
        id: id + '_resize_se',
        'class': 'action resize resize-se',
        tabindex: '-1'
      }]]]]);
      if (f.bigicon) $('#' + id + '_bigicon').addClass(f.bigicon);
      if (f.chromeless) $('#' + id + '_container').addClass('chromeless');
      if (f.resizable) $('#' + id + '_container').addClass('resizable');
      ...
      if (v = f.title) t.setTitle(v);
      if (v = f.content) t.setContent(v);
      if (v = f.url) t.setURL(v);
      ...
      function handleAction(e) {
        ...
        if (el[0].nodeName == 'A' && el.hasClass('action')) {
          action = $.grep(el[0].className.split(/\s+/), function (v) {
            return !/^action|resize$/.test(v);
          }).join(' ');
          if (e.type == 'mousedown') f = t.mouseDownActions[action];else {
  ...
  $.WindowManager = {
    ...
    setup: function setup() {
      var w = window,
          lw,
          nwm; // Find root window manager

      while ((w = w.parent || w.opener) && w != lw) {
        if (w.$ && w.$.WindowManager) nwm = w.$.WindowManager;
        lw = w;
      ...
      if (nwm && nwm != this) {
        ...
        $(window).bind('resize', function () {
          $($.WindowManager.windows).each(function (i, w) {
            if (w.lastRect) w.resizeTo($.winWidth(), $.winHeight(), 1);
          });
        });
      }
    },
    find: function find(tw) {
      var i, ifr, w;

      for (i = 0; i < this.windows.length; i++) {
        w = this.windows[i];
        ifr = $('#' + w.id + '_ifr')[0];
        if (ifr && ifr.contentWindow == tw) return w;
      }

      return this.defaultWin;
    },
    open: function open(f, a) {
      var w = new $.Window(f, a).render();
      this.windows.push(w);
      return w.focus();
    },
    warn: function warn(msg, cb) {
    ...
    confirm: function confirm(msg, cb) {
      ...
      }, cb).setContent('<div class="message">' + msg + '</div>' + '<a href="" class="action ok">' + this.i18n.yes + '</a>' + '<a href="#" class="action cancel">' + this.i18n.no + '</a>');
    },
    modalBox: function modalBox(msg, f, cb) {
      f = $.extend({
        type: 'alert',
        bigicon: 'error',
        width: 400,
        ...
        onaction: function onaction(a) {
          if (a == 'ok') {
            this.close();
            if (cb) cb.call(this, 1);
          } else if (a == 'close') this.close();
        }
      }, f);
      return this.open(f).setContent('<div class="message">' + (msg || '') + '</div>' + '<a href="" class="action ok">' + this.i18n.ok + '</a>');
    },
    remove: function remove(w) {
      var t = this,
          fr,
          z = 0,
          nl = []; // Find front most window
