...
(function ($) {
  ...
  if (!$.multiUpload.initialized && isInstalled('2.0.31005.0')) {
    ...
    $.multiUpload.runtime = 'silverlight'; // Register global Silverlight instance
    ...
    $.extend($.multiUpload.prototype, {
      init: function init() {
        var up = this,
            sel = [];
        $.multiUpload.instances.push(up); // Add silverlight runtime

        if (!$('#multiuploader')[0]) {
          $("body").append('<object id="multiuploader" data="data:application/x-silverlight," type="application/x-silverlight-2" width="100" height="100">' + '<param name="source" value="' + up.settings.silverlight_xap_url + '"/>' + '<param name="onerror" value="onSilverlightError" /></object>');
        ...
        $(up).bind('multiUpload:slSelectSuccessful', function () {
          $(up).trigger('multiUpload:filesSelected', [{
            files: sel
          }]);
        ...
        $(up).bind('multiUpload:slUploadFileProgress', function (e, id, lod, tot) {
          ...
          file.loaded = lod;
          $(up).trigger('multiUpload:fileUploadProgress', [{
            file: file,
            loaded: lod,
            total: tot
          }]);
        ...
        $(up).bind('multiUpload:slUploadSuccessful', function (e, id, resp) {
          ...
          if (fo = up.getFile(id)) {
            if (!fo.status) {
              fo.status = "completed";
              $(up).trigger('multiUpload:fileUploaded', [{
                file: fo,
                response: resp
              }]);
            }
        ...
        $(up).bind('multiUpload:slUploadChunkError', function (e, id, chunk, chunks, err) {
          $(up).trigger('multiUpload:uploadChunkError', [{
            file: up.getFile(id),
            chunk: chunk,
            chunks: chunks,
            error: err
        ...
        $(up).bind('multiUpload:selectFiles', function (e) {
          $.multiUpload.plugin.SelectFiles('Files |' + $.map(up.settings.filter, function (v) {
            return '*.' + v;
          }).join(';'));
        });
        ...
        $(up).bind('multiUpload:uploadFile', function (e, fo) {
          $.multiUpload.plugin.UploadFile(fo.id, up.settings.upload_url + '&name=' + escape(fo.name) + "&path=" + escape(up.settings.path), parseInt(up.settings.chunk_size));
        });
