...
(function ($) {
  ...
  $.multiUpload = function (s) {
    var up = this,
        mul;
    up.id = up.generateID(); // Default settings

    up.settings = {};
    up.settings = s = $.extend(up.settings, s); // Parse max size

    if (s.max_size) s.max_size = up.parseSize(s.max_size); // Parse chunk size

    if (s.chunk_size) s.chunk_size = up.parseSize(s.chunk_size);
    ...
    $(['setup', 'filesSelected', 'fileProgress', 'filesProgress', 'filesUploaded', 'fileUploaded', 'fileUploadProgress']).each(function () {
      if (s[this]) $(up).bind('multiUpload:' + this, s[this]);
    });
    $(up).trigger('multiUpload:setup');
    $(up).bind('multiUpload:selectFiles', function () {
      this.cache = {};
    });
  ...
  $.extend($.multiUpload.prototype, {
    ...
    uploadNext: function uploadNext() {
      var i,
          fl = this.files;
    ...
    getFile: function getFile(id) {
      var t = this,
          ...
          i,
          fl = t.files;
      if (f = t.cache[id]) return f;

      for (i = 0; i < fl.length; i++) {
        if (fl[i].id == id) return t.cache[id] = fl[i];
      }
    },
    ...
    formatSize: function formatSize(v) {
      // MB
      if (v > 1048576) return Math.round(v / 1048576, 1) + " MB"; // KB

      if (v > 1024) return Math.round(v / 1024, 1) + " KB";
      return v + " b";
    },
    generateID: function generateID() {
      return 'u' + id++;
    },
    parseSize: function parseSize(sz) {
      var mul;
  ...
  $.extend($.multiUpload, {
    ...
    remove: function remove(id) {
      if (this.get(id)) delete this.instances[id];
    },
    add: function add(up) {
