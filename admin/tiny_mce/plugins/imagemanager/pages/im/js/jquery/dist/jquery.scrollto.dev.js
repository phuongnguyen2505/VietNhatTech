...
(function ($) {
  ...
  $.fn.scrollTo = function (target, duration, settings) {
    ...
    duration = duration || settings.speed || settings.duration; // Make sure the settings are given right
    ...
    return this.scrollable().each(function () {
      ...
      $.each(settings.axis.split(''), function (i, axis) {
        ...
        if (toff) {
          // jQuery / DOMElement
          attr[key] = toff[pos] + (win ? 0 : old - $elem.offset()[pos]); // If it's a dom element, reduce the margin
        ...
        } else attr[key] = targ[pos]; // Number or 'number'


        if (/^\d+$/.test(attr[key])) // Check the limits
          attr[key] = attr[key] <= 0 ? 0 : Math.min(attr[key], max(Dim)); // Queueing axes
