...
(function ($) {
  ...
  $.template.helpers = $.template.instance.prototype.helpers = {
    substr: function substr(value, start, length) {
  ...
  $.extend($.template.instance.prototype, {
    ...
    apply: function apply(values) {
      if (this.options.compile) {
        ...
        var fn = function fn(m, name, format, args) {
          if (format) {
  ...
  $.fn.domManip = function (args, table, reverse, callback) {
    if (args[0].isTemplate) {
      ...
      args[0] = args[0].apply(args[1]); // Get rid of the arguements, we don't want to pass them on
