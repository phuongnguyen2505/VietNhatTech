...
(function ($) {
  ...
  function DropMenu(s) {
    ...
    t.id = s.id;
    if (s.setup) s.setup.call(t, t); // Private methods

    function hideHandler(e) {
      if (e.target.nodeName !== 'A' || !$(e.target).parents('div.' + s.menu_class).length) t.hide();
    }
    ...
    function clickHandler(e) {
      ...
      if (e.target.nodeName === 'A') {
        ...
        if (!m.show && !m.disabled) {
          if (m.onclick) m.onclick.call(m.scope || this, e, m);
          if (s.onClick) s.onClick(e, m);
          m = t;
    ...
    $.extend(this, {
      createMenu: function createMenu(s) {
      ...
      find: function find(n) {
        ...
        for (i = 0; i < t.items.length; i++) {
          if (t.items[i].id === n) return t.items[i];
        }
      ...
      render: function render(n) {
        if (!n.id) n.id = t.id = uniqueId();else t.id = 'jquery_mc_' + n.id;
        if (s.onInit) s.onInit(t);
      },
      show: function show(x, y) {
        ...
        if (!t.rendered) {
          ...
          $.each(t.items, function (i, v) {
            ...
            if (v.constructor == DropMenu) {
              ...
              if (v.settings['class']) cl += ' ' + v.settings['class'];
            } else {
              ti = v.title;
              if (this['class']) cl = ' ' + v['class'];
            }

            if (v.disabled || v.settings && v.settings.disabled) cl += ' disabled'; // Add menu item

            it = createElm('div', {
              id: t.id + '_' + v.id,
              'class': s.menu_item_class + cl
            });
            an = createElm('a', {
              rel: v.id,
              href: '#'
            }, ti); //$(an).append(createElm('span', null, ti));
        ...
        $().mouseup(hideHandler);
        m = $('#' + t.id);
        m.mouseover(mouseOverHandler).show().css({
          left: -5000,
          top: -5000
        }); // Measure and align

        if (s.halign == 'right') x -= m.width();
        if (s.valign == 'bottom') y -= m.height(); // Constrain
      ...
      addSeparator: function addSeparator() {
        return t.add({
          'class': s.separator_class,
          title: 'separator'
        });
      },
      addMenu: function addMenu(o) {
        if (!o.onClick) o.onClick = s.onClick;
        o = new DropMenu(o);
        o.parentMenu = t;
        return t.add(o);
      }
