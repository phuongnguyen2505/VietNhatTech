...
(function () {
  window.mcImageManager = {
    ...
    setup: function setup() {
      var t = this,
          o,
          d = document,
          cp = []; // Find document_base_url

      o = d.location.href;
      if (o.indexOf('?') != -1) o = o.substring(0, o.indexOf('?'));
      o = o.substring(0, o.lastIndexOf('/') + 1);
      t.settings.default_base_url = unescape(o); // Find script base URL
      ...
      function get(nl) {
        ...
        for (i = 0; i < nl.length; i++) {
          n = nl[i];
          cp.push(n);
          if (n.src && /mcimagemanager\.js/g.test(n.src)) return n.src.substring(0, n.src.lastIndexOf('/'));
        }
      }

      ;
      o = d.documentElement;
      if (o && (o = get(o.getElementsByTagName('script')))) return t.baseURL = o;
      o = d.getElementsByTagName('script');
      if (o && (o = get(o))) return t.baseURL = o;
      o = d.getElementsByTagName('head')[0];
      if (o && (o = get(o.getElementsByTagName('script')))) return t.baseURL = o;
    },
    relaxDomain: function relaxDomain() {
      var t = this,
          p = /(http|https):\/\/([^\/:]+)\/?/.exec(t.baseURL); // Use tinymce relaxed domain
      ...
      if (p && p[2] != document.location.hostname) document.domain = t.relaxedDomain = p[2].replace(/.*\.(.+\..+)$/, '$1');
    },
    init: function init(s) {
      this.extend(this.settings, s);
    },
    browse: function browse(s) {
      var t = this;
      s = s || {};
      ...
      if (s.fields) {
        s.oninsert = function (o) {
          t.each(s.fields.replace(/\s+/g, '').split(/,/), function (v) {
            var n;
            if (n = document.getElementById(v)) t._setVal(n, o.focusedFile.url);
          });
        };
      }
    ...
    createDir: function createDir(s) {
      this.openWin({
        page: 'createdir.html',
        width: 450,
        height: 280
    ...
    openWin: function openWin(f, a) {
      ...
      if (f.x == -1) f.x = parseInt(screen.width / 2.0) - f.width / 2.0;
      if (f.y == -1) f.y = parseInt(screen.height / 2.0) - f.height / 2.0;
      if (f.page) f.url = t.baseURL + '/../index.php?type=im&page=' + f.page;
      if (a.session_id) f.url += '&sessionid=' + a.session_id;
      if (a.custom_data) f.url += '&custom_data=' + escape(a.custom_data);
      if (t.relaxedDomain) f.url += '&domain=' + escape(t.relaxedDomain); // Open in specified frame

      if (a.target_frame) {
        if (v = frames[a.target_frame]) v.document.location = f.url;
        if (v = document.getElementById(a.target_frame)) v.src = f.url;
        return;
      } // Use TinyMCE window API


      if (window.tinymce && tinyMCE.activeEditor) return tinyMCE.activeEditor.windowManager.open(f, a); // Use jQuery WindowManager

      if (window.jQuery && jQuery.WindowManager) return jQuery.WindowManager.open(f, a); // Use native dialogs

      w = window.open(f.url, 'mcImageManagerWin', 'left=' + f.x + ',top=' + f.y + ',width=' + f.width + ',height=' + f.height + ',scrollbars=' + (f.scrollbars ? 'yes' : 'no') + ',resizable=' + (f.resizable ? 'yes' : 'no') + ',statusbar=' + (f.statusbar ? 'yes' : 'no'));
    ...
    each: function each(o, f, s) {
      ...
      if (o) {
        ...
        if (o.length !== undefined) {
          ...
          for (n in o) {
            if (o.hasOwnProperty(n)) f.call(s, o[n], n, o);
          }
        }
      }
    },
    ...
    // Legacy functions
    open: function open(fn, en, url, cb, s) {
      var t = this,
          el;
      s = s || {}; // Use input value if it was found

      if (!s.url && document.forms[fn] && (el = document.forms[fn].elements[en.split(',')[0]])) s.url = el.value;
      ...
      if (!cb) {
        s.oninsert = function (o) {
          ...
          for (i = 0; i < v.length; i++) {
            if (e = document.forms[fn][v[i]]) t._setVal(e, f.url);
          }
        };
      } else {
    ...
    filebrowserCallBack: function filebrowserCallBack(fn, u, ty, w, ask) {
      var t = mcImageManager,
          i,
          hl,
          fo,
          s = {}; // Is filemanager included, ask it first
      ...
      if (window.mcFileManager && !ask) {
        ...
        if (fo && mcFileManager.filebrowserCallBack(fn, u, ty, w, 1)) return;
      ...
      t.each(tinyMCE.activeEditor ? tinyMCE.activeEditor.settings : tinyMCE.settings, function (v, k) {
        if (k.indexOf('imagemanager_') === 0) s[k.substring(13)] = v;
      });
      t.browse(t.extend(s, {
        url: w.document.forms[0][fn].value,
        relative_urls: 0,
        oninsert: function oninsert(o) {
          var f, u, na;
          f = w.document.forms[0];
          u = o.focusedFile.url;
          inf = o.focusedFile.custom; // Let TinyMCE convert the URLs

          if (typeof TinyMCE_convertURL != "undefined") u = TinyMCE_convertURL(u, null, true);else if (tinyMCE.convertURL) u = tinyMCE.convertURL(u, null, true);else u = tinyMCE.activeEditor.convertURL(u, null, true); // Set alt and title info

          if (inf.custom && inf.custom.description) {
            na = ['alt', 'title', 'linktitle'];

            for (i = 0; i < na.length; i++) {
              if (f.elements[na[i]]) f.elements[na[i]].value = inf.custom.description;
            }
          }
  ...
  var mcImageManagerPlugin = {
    ...
    convertURL: function convertURL(u) {
      // 2.x
      if (window.TinyMCE_convertURL) return TinyMCE_convertURL(u, null, true); // 2.x

      if (tinyMCE.convertURL) return tinyMCE.convertURL(u, null, true); // 3.x

      return tinyMCE.activeEditor.convertURL(u, null, true);
    },
    replace: function replace(t, d, e) {
      var i, r;
      if (typeof t != 'string') return t(d, e);
      ...
      t = '' + t.replace(/\{\$([^\}]+)\}/g, function (a, b) {
        var l = b.split('|'),
            v = get(d, l[0]); // Default encoding

        if (l.length == 1 && e && e.xmlEncode) v = e.xmlEncode(v); // Execute encoders
    ...
    tinymce.create('tinymce.plugins.ImageManagerPlugin', {
      init: function init(ed, url) {
        ...
        ed.addCommand('mceInsertImage', function (u, v) {
          var s = {}; // Grab imagemanager prefixed options
          ...
          mcImageManager.browse(tinymce.extend(s, {
            oninsert: function oninsert(o) {
              ...
              ed.execCommand('mceInsertContent', false, mcImageManagerPlugin.replace(ed.getParam('imagemanager_insert_template', '<img src="{$url}" width="{$custom.width}" height="{$custom.height}" />'), o.focusedFile, {
                urlencode: function urlencode(v) {
                  return escape(v);
                },
                xmlEncode: function xmlEncode(v) {
          ...
          case 'insertimage':
            ...
            if (v instanceof Array) {
              ...
              c = cm.createButton('insertimage', {
                ...
                image: t.url + '/pages/im/img/insertimage.gif',
                onclick: function onclick() {
                  ed.execCommand('mceInsertImage', false, {
                    template: v
                  });
                }
              });
  ...
  if (window.TinyMCE_Engine) {
    var TinyMCE_ImageManagerPlugin = {
      ...
      execCommand: function execCommand(id, el, cmd, ui, v) {
        ...
        if (cmd == 'mceInsertImage') {
          mcImageManager.browse(tinyMCE.extend({
            ...
            oninsert: function oninsert(o) {
              ...
              ed.execCommand('mceInsertContent', false, mcImageManagerPlugin.replace(tinyMCE.getParam('imagemanager_insert_template', '<a href="{$url}" rel="lightbox"><img src="{$custom.thumbnail_url}" width="{$custom.twidth}" height="{$custom.theight}" /></a>'), o.focusedFile, {
                urlencode: function urlencode(v) {
                  return escape(v);
                },
                xmlEncode: function xmlEncode(v) {
