...
var LinkDialog = {
  ...
  init: function init() {
    var f = document.forms[0],
        ed = tinyMCEPopup.editor; // Setup browse button
  ...
  update: function update() {
    ...
    e = ed.dom.getParent(ed.selection.getNode(), 'A'); // Remove element if there is no href
  ...
  checkPrefix: function checkPrefix(n) {
  ...
  fillTargetList: function fillTargetList(id) {
    var dom = tinyMCEPopup.dom,
        lst = dom.get(id),
        v;
    lst.options[lst.options.length] = new Option(tinyMCEPopup.getLang('not_set'), '');
